...
final class AVORemotePushManager: NSObject, ObservableObject {
    ...
    func configureAndRegister() {
        print("AVO PUSH BUILD54: configureAndRegister")
        ...
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound, .badge, .timeSensitive]) { granted, error in
            DispatchQueue.main.async {
                ...
                if let error {
                    ...
                    print("AVO PUSH BUILD54 permission error:", error.localizedDescription)
                    return
                }

                if granted {
                    self.setStatus("PUSH PERMISSION OK - REQUESTING APNS TOKEN")
                    print("AVO PUSH BUILD54 permission OK")
                    self.registerForRemoteNotificationsOnce(force: true)
                } else {
                    self.setStatus("PUSH PERMISSION DENIED")
                    print("AVO PUSH BUILD54 permission denied")
                }
            }
        }
    }
    ...
    func registerForRemoteNotificationsOnce(force: Bool = false) {
        ...
        DispatchQueue.main.async {
            print("AVO PUSH BUILD54: UIApplication.registerForRemoteNotifications()")
    ...
    func updateDeviceToken(_ deviceToken: Data) {
        ...
        print("AVO PUSH BUILD54 APNS TOKEN:", token)
    ...
    private func uploadStoredTokenIfAvailableAsync() async {
        ...
        guard !stored.isEmpty else {
            print("AVO PUSH BUILD54: no stored APNS token yet")
            return
        }
    ...
    func refreshServerStatus() async {
        ...
        do {
            ...
            await MainActor.run {
                ...
                print("AVO PUSH BUILD54 server status:", self.lastServerStatus)
            }
        } catch {
            await MainActor.run {
                self.lastServerStatus = "PUSH SERVER ERROR: \(error.localizedDescription)"
                UserDefaults.standard.set(self.lastServerStatus, forKey: "AVO_PUSH_SERVER_STATUS")
                print("AVO PUSH BUILD54 server status error:", error.localizedDescription)
            }
        }
    }
    ...
    private func uploadToken(_ token: String, reason: String) async {
        if uploadInProgress {
            print("AVO PUSH BUILD54: upload already in progress")
            return
        }
        ...
        let payload: [String: Any] = [
            ...
            "appVersion": Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "1.3.9",
            "build": Bundle.main.infoDictionary?["CFBundleVersion"] as? String ?? "54",
            "reason": reason
        ]

        do {
            request.httpBody = try JSONSerialization.data(withJSONObject: payload, options: [])
            print("AVO PUSH BUILD54 uploading APNS token to:", registerURL.absoluteString, "reason:", reason)
            ...
            await MainActor.run {
                if (200...299).contains(code) {
                    ...
                    self.setStatus("PUSH TOKEN REGISTERED BUILD54 HTTP \(code)")
                    print("AVO PUSH BUILD54 REGISTER OK:", text)
                } else {
                    self.setStatus("PUSH TOKEN ERROR BUILD54 HTTP \(code): \(text.prefix(160))")
                    print("AVO PUSH BUILD54 REGISTER HTTP ERROR \(code):", text)
                }
            }
        } catch {
            await MainActor.run {
                self.setStatus("PUSH TOKEN UPLOAD ERROR BUILD54: \(error.localizedDescription)")
                print("AVO PUSH BUILD54 REGISTER ERROR:", error.localizedDescription)
            }
        }
    }
...
final class AVOPushAppDelegate: NSObject, UIApplicationDelegate {
    func application(
        ...
        print("AVO PUSH BUILD54: didFinishLaunching")
    ...
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        print("AVO PUSH BUILD54: didRegisterForRemoteNotificationsWithDeviceToken")
        Task { @MainActor in
    ...
    func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
        print("AVO PUSH BUILD54 APNs registration failed:", error.localizedDescription)
        Task { @MainActor in
            AVORemotePushManager.shared.configureAndRegister()
        }
    }
    ...
    func application(
        ...
        print("AVO PUSH BUILD54 remote push received:", userInfo)
