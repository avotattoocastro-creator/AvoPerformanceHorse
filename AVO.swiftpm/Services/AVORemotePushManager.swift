...
final class AVORemotePushManager: NSObject, ObservableObject {
    ...
    func configureAndRegister() {
        print("AVO PUSH BUILD51: configureAndRegister")
        ...
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .sound, .badge, .timeSensitive]) { granted, error in
            DispatchQueue.main.async {
                ...
                if let error {
                    ...
                    print("AVO PUSH BUILD51 permission error:", error.localizedDescription)
                    return
                }

                if granted {
                    self.setStatus("PUSH PERMISSION OK - REQUESTING APNS TOKEN")
                    print("AVO PUSH BUILD51 permission OK")
                    self.registerForRemoteNotificationsOnce(force: true)
                } else {
                    self.setStatus("PUSH PERMISSION DENIED")
                    print("AVO PUSH BUILD51 permission denied")
                }
            }
        }
    }
    ...
    func registerForRemoteNotificationsOnce(force: Bool = false) {
        ...
        DispatchQueue.main.async {
            print("AVO PUSH BUILD51: UIApplication.registerForRemoteNotifications()")
    ...
    func updateDeviceToken(_ deviceToken: Data) {
        ...
        print("AVO PUSH BUILD51 APNS TOKEN:", token)
    ...
    private func uploadStoredTokenIfAvailableAsync() async {
        ...
        guard !stored.isEmpty else {
            print("AVO PUSH BUILD51: no stored APNS token yet")
            return
        }
    ...
    func refreshServerStatus() async {
        ...
        do {
            ...
            await MainActor.run {
                ...
                print("AVO PUSH BUILD51 server status:", self.lastServerStatus)
            }
        } catch {
            await MainActor.run {
                self.lastServerStatus = "PUSH SERVER ERROR: \(error.localizedDescription)"
                UserDefaults.standard.set(self.lastServerStatus, forKey: "AVO_PUSH_SERVER_STATUS")
                print("AVO PUSH BUILD51 server status error:", error.localizedDescription)
            }
        }
    }
    ...
    private func uploadToken(_ token: String, reason: String) async {
        if uploadInProgress {
            print("AVO PUSH BUILD51: upload already in progress")
            return
        }
        ...
        do {
            ...
            print("AVO PUSH BUILD51 uploading APNS token to:", registerURL.absoluteString, "reason:", reason)
            ...
            await MainActor.run {
                if (200...299).contains(code) {
                    ...
                    self.setStatus("PUSH TOKEN REGISTERED BUILD51 HTTP \(code)")
                    print("AVO PUSH BUILD51 REGISTER OK:", text)
                } else {
                    self.setStatus("PUSH TOKEN ERROR BUILD51 HTTP \(code): \(text.prefix(160))")
                    print("AVO PUSH BUILD51 REGISTER HTTP ERROR \(code):", text)
                }
            }
        } catch {
            await MainActor.run {
                self.setStatus("PUSH TOKEN UPLOAD ERROR BUILD51: \(error.localizedDescription)")
                print("AVO PUSH BUILD51 REGISTER ERROR:", error.localizedDescription)
            }
        }
    }
...
final class AVOPushAppDelegate: NSObject, UIApplicationDelegate {
    func application(
        ...
        print("AVO PUSH BUILD51: didFinishLaunching")
    ...
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        print("AVO PUSH BUILD51: didRegisterForRemoteNotificationsWithDeviceToken")
        Task { @MainActor in
    ...
    func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
        print("AVO PUSH BUILD51 APNs registration failed:", error.localizedDescription)
        Task { @MainActor in
            AVORemotePushManager.shared.configureAndRegister()
        }
    }
    ...
    func application(
        ...
        print("AVO PUSH BUILD51 remote push received:", userInfo)
