...
final class AVOHardwareReceiver: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    private func postFixedCloudGeofence(payload: [String: Any]) async throws -> (json: [String: Any]?, text: String, code: Int) {
        guard let url = URL(string: "https://live.avoperformance.org/api/geofence/save") else {
            throw NSError(domain: "AVO.Raspberry", code: -1, userInfo: [NSLocalizedDescriptionKey: "BAD FIXED GEOFENCE URL"])
        }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        ...
        let text = String(data: data, encoding: .utf8) ?? ""
        guard (200...299).contains(code) else {
            throw NSError(domain: "AVO.Raspberry", code: code, userInfo: [NSLocalizedDescriptionKey: "HTTP \(code) \(text.prefix(160))"])
        }
        let obj = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any]
        if let obj, let ok = obj["ok"] as? Bool, ok == false {
            throw NSError(domain: "AVO.Raspberry", code: code, userInfo: [NSLocalizedDescriptionKey: "SERVER OK FALSE \(text.prefix(160))"])
        }
        return (obj, text, code)
    }
