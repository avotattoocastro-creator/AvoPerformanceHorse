...
final class AVOHardwareReceiver: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    @Published var serverVestState = "WAITING"
    @Published var serverVestLabel = "CHALECO ESPERANDO"
    @Published var serverVestConnected = false
    @Published var serverVestFrozen = false
    @Published var serverVestDisconnected = true
    @Published var serverVestAgeSeconds = -1.0
    @Published var serverVestLastTelemetryAt = "--"
    ...
    @Published var serverGeofenceStatus = "SERVER GEOFENCE WAITING"
    ...
    @Published var cloudAPI = "https://live.avoperformance.org/api/latest/compact"
    ...
    @Published var gpsRtcmBytes = 0
    @Published var gpsCarrSoln = -1
    @Published var rtkLevel = "--"
    @Published var rtkProxyStatus = "--"
    @Published var rtkProxyConnected = false
    @Published var rawRecordingActive = false
    @Published var rawRecordingSDOK = false
    @Published var rawRecordingSessionID = "--"
    ...
    func sendTrainingZone(_ zone: TrainingZone, horseName: String) {
        ...
        // Campo real: primero se guarda sí o sí en el servidor cloud.
        // BLE queda como extra/fallback para el chaleco, pero la fuente principal es el servidor.
        saveTrainingZoneToServer(zone, horseName: horseName)
    ...
    func saveTrainingZoneToServer(_ zone: TrainingZone, horseName: String) {
        ...
        sendTrainingZoneToCloud(payload)
    }

    private func sendTrainingZoneToCloud(_ payload: [String: Any]) {
        serverGeofenceStatus = "SERVER GEOFENCE SENDING"
        geofenceStatus = "GEOFENCE SENDING SERVER"
        Task { [weak self] in
            guard let self else { return }
            do {
                let result = try await self.postFixedCloudGeofence(payload: payload)
                await MainActor.run {
                    self.serverGeofenceStatus = "SERVER GEOFENCE SAVED HTTP \(result.code)"
                    self.geofenceStatus = "GEOFENCE SAVED SERVER"
                    self.cloudLastHTTPCode = result.code
                    self.cloudLastPayload = String(result.text.prefix(240))
                    self.cloudLastError = "--"
                }
            } catch {
                await MainActor.run {
                    self.serverGeofenceStatus = "SERVER GEOFENCE ERROR"
                    self.geofenceStatus = "SERVER SAVE ERROR"
                    self.cloudLastError = error.localizedDescription
                    self.emitTrainingEvent(type: "geofence.server.error", title: "Geocerca no guardada en servidor", body: error.localizedDescription)
                }
            }
        }
    }
    ...
    private func applyCloudPacket(_ json: [String: Any], rawText: String, httpCode: Int) {
        ...
        // /api/latest/compact is the source of truth for the Live Dashboard.
        // If the payload contains connection/gps/rtk/geofence/rawRecording blocks,
        // never fall back to local/demo states. Those compact flags are authoritative.
        if json["vestStatus"] != nil || json["connection"] != nil || json["proState"] != nil || json["gps"] != nil || json["rtk"] != nil || json["rawRecording"] != nil {
            parseJSON(json)
    ...
    private func postRaspberryJSON(path: String, payload: [String: Any]) async throws -> (json: [String: Any]?, text: String, code: Int) {
        guard let url = raspberryAPIURL(path: path) else {
            throw NSError(domain: "AVO.Raspberry", code: -1, userInfo: [NSLocalizedDescriptionKey: "BAD POST URL"])
        }
        var request = URLRequest(url: url)
        ...
        guard (200...299).contains(code) else {
            throw NSError(domain: "AVO.Raspberry", code: code, userInfo: [NSLocalizedDescriptionKey: "HTTP \(code) \(text.prefix(120))"])
        }
        let obj = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any]
        return (obj, text, code)
    }

    private func cloudGeofenceSaveURL() -> URL? {
        // Usa exactamente el mismo host/esquema/PUERTO que funciona en el dashboard (/api/latest).
        // En tu Raspberry la app conecta bien con https://live.avoperformance.org:443/api/latest,
        // por eso el guardado de geocerca no debe usar una URL hardcodeada sin respetar ese puerto.
        if let latestURL = URL(string: cloudAPI),
           var components = URLComponents(url: latestURL, resolvingAgainstBaseURL: false) {
            components.path = "/api/geofence/save"
            components.query = nil
            components.fragment = nil
            if components.scheme == "https" && components.port == nil {
                components.port = 443
            }
            return components.url
        }
        return URL(string: "https://live.avoperformance.org:443/api/geofence/save")
    }

    private func postFixedCloudGeofence(payload: [String: Any]) async throws -> (json: [String: Any]?, text: String, code: Int) {
        guard let url = cloudGeofenceSaveURL() else {
            throw NSError(domain: "AVO.Raspberry", code: -1, userInfo: [NSLocalizedDescriptionKey: "BAD GEOFENCE SAVE URL"])
        }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        ...
        request.setValue("AVO2026", forHTTPHeaderField: "X-AVO-Token")
        request.setValue("ios_geofence_save", forHTTPHeaderField: "X-AVO-Source")
        ...
        let text = String(data: data, encoding: .utf8) ?? ""
        guard (200...299).contains(code) else {
            throw NSError(domain: "AVO.Raspberry", code: code, userInfo: [NSLocalizedDescriptionKey: "HTTP \(code) URL=\(url.absoluteString) BODY=\(text.prefix(160))"])
        }
        let obj = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any]
        if let obj, let ok = obj["ok"] as? Bool, ok == false {
            throw NSError(domain: "AVO.Raspberry", code: code, userInfo: [NSLocalizedDescriptionKey: "SERVER OK FALSE URL=\(url.absoluteString) BODY=\(text.prefix(160))"])
        }
        return (obj, text, code)
    }
    ...
    private func fetchRaspberryLatest() async {
        ...
        do {
            // v1.2.2 BUILD35: /api/latest es la única verdad para el dashboard.
            ...
            // Registro opcional, solo diagnóstico. Nunca cambia ONLINE/OFFLINE del dashboard.
            if Date().timeIntervalSince(lastRegistryFetchDate) > 10.0, let registryURL = raspberryAPIURL(path: "/api/vests") {
                if let registry = try? await fetchRaspberryJSON(url: registryURL) {
            ...
            await MainActor.run {
                ...
                self.cloudLastError = error.localizedDescription
                self.markVestDisconnected(reason: "cloud offline")
            }
        }
    }
    ...
    private func applyGPSDictionary(_ gps: [String: Any]) {
        ...
        if let rtcm = jsonInt(gps["rtcmBytes"]) ?? jsonInt(gps["rtcm_bytes"]) { gpsRtcmBytes = rtcm }
        if let carr = jsonInt(gps["carrSoln"]) ?? jsonInt(gps["carr_soln"]) ?? jsonInt(gps["carrierSolution"]) { gpsCarrSoln = carr }
        if let hacc = jsonDouble(gps["hAccM"]) ?? jsonDouble(gps["haccM"]) ?? jsonDouble(gps["horizontalAccuracyM"]) { gpsHDOP = hacc }
        else if let hdop = jsonDouble(gps["hdop"]) ?? jsonDouble(gps["accuracy"]) ?? jsonDouble(gps["accuracy_m"]) { gpsHDOP = hdop }
        if let altitude = jsonDouble(gps["altitude"]) ?? jsonDouble(gps["alt"]) { gpsAltitude = altitude }
    }

    private func applyServerVestStatusFromLatest(_ json: [String: Any]) {
        let vestStatus = json["vestStatus"] as? [String: Any]
        let connection = json["connection"] as? [String: Any]
        let proState = json["proState"] as? [String: Any]

        let stateRaw =
            (connection?["state"] as? String) ??
            (connection?["vestState"] as? String) ??
            (vestStatus?["state"] as? String) ??
            (proState?["vestState"] as? String) ??
            (json["vestState"] as? String) ??
            (json["state"] as? String) ??
            "CONNECTED"

        let state = stateRaw.uppercased()
        let compactDataFrozen = jsonBool(connection?["dataFrozen"])
        let compactDisconnected = jsonBool(connection?["disconnected"])
        let compactAge = jsonDouble(connection?["ageSeconds"])

        let frozen = compactDataFrozen ?? jsonBool(vestStatus?["frozen"]) ?? jsonBool(proState?["dataFrozen"]) ?? state.contains("FROZEN")
        let disconnected = compactDisconnected ?? jsonBool(vestStatus?["disconnected"]) ?? jsonBool(proState?["vestDisconnected"]) ?? state.contains("DISCONNECTED")
        let connectedFlag = jsonBool(connection?["connected"]) ?? jsonBool(vestStatus?["connected"]) ?? jsonBool(proState?["vestConnected"])
        let connected = connectedFlag ?? (!frozen && !disconnected && (state.contains("CONNECTED") || state.contains("ONLINE") || state == "OK" || state == "LIVE"))
        let age = compactAge ?? jsonDouble(vestStatus?["ageSeconds"]) ?? jsonDouble(proState?["lastTelemetryAgeSeconds"]) ?? lastVestSeenSeconds
        let label = (vestStatus?["label"] as? String) ?? (connection?["label"] as? String)

        // Critical rule: when compact says dataFrozen=false and disconnected=false,
        // the app must show LIVE/CONNECTED even if local calculated/demo values did not change.
        serverVestState = disconnected ? "DISCONNECTED" : frozen ? "FROZEN" : connected ? "CONNECTED" : state
        serverVestLabel = label ?? (disconnected ? "CHALECO DESCONECTADO" : frozen ? "CHALECO CONGELADO" : connected ? "CHALECO CONECTADO" : "CHALECO ESPERANDO")
        serverVestConnected = connected && !disconnected && !frozen
        serverVestFrozen = frozen && !disconnected
        serverVestDisconnected = disconnected
        serverVestAgeSeconds = age
        if let lastAt = vestStatus?["lastTelemetryAt"] as? String { serverVestLastTelemetryAt = lastAt }
        lastVestSeenSeconds = age

        if serverVestDisconnected {
            vestIsConnected = false
            vestConnectionState = "VEST DISCONNECTED"
            vestConnectionAlert = serverVestLabel
            vestRegistryStatus = "VEST DISCONNECTED"
            cloudStatus = "CLOUD ONLINE"
        } else if serverVestFrozen {
            vestIsConnected = false
            vestConnectionState = "VEST FROZEN"
            vestConnectionAlert = serverVestLabel
            vestRegistryStatus = "VEST FROZEN"
            cloudStatus = "CLOUD ONLINE"
        } else if serverVestConnected {
            vestIsConnected = true
            vestConnectionState = "VEST CONNECTED · CLOUD"
            vestConnectionAlert = serverVestLabel
            vestRegistryStatus = "VEST ONLINE"
            cloudStatus = "CLOUD ONLINE"
            lastPacketDate = Date()
    ...
    private func parseJSON(_ json: [String: Any]) {
        ...
        applyServerVestStatusFromLatest(json)

        if let rtk = json["rtk"] as? [String: Any] {
            if let level = (rtk["level"] as? String) ?? (rtk["state"] as? String) { rtkLevel = level }
            if let status = (rtk["proxyStatus"] as? String) ?? (rtk["proxy_status"] as? String) { rtkProxyStatus = status }
            if let ok = jsonBool(rtk["proxyConnected"]) ?? jsonBool(rtk["proxy_connected"]) { rtkProxyConnected = ok }
        }

        if let geofence = json["geofence"] as? [String: Any] {
            if let inside = jsonBool(geofence["inside"]) { isInsideTrainingZone = inside }
            let state = ((geofence["state"] as? String) ?? (isInsideTrainingZone ? "INSIDE_ZONE" : "OUTSIDE_ZONE")).uppercased()
            if let distance = jsonDouble(geofence["distanceM"]) ?? jsonDouble(geofence["distance_m"]) {
                trainingZonePresence = String(format: "%@ · %.0fm", state, distance)
            } else {
                trainingZonePresence = state
            }
        }

        if let raw = json["rawRecording"] as? [String: Any] {
            if let active = jsonBool(raw["active"]) { rawRecordingActive = active }
            if let sd = jsonBool(raw["sdOK"]) ?? jsonBool(raw["sd_ok"]) { rawRecordingSDOK = sd }
            if let sid = (raw["sessionId"] as? String) ?? (raw["session_id"] as? String) { rawRecordingSessionID = sid }
        }

        if let battery = jsonInt(json["battery"]) ?? jsonInt(json["batteryPercent"]) ?? jsonInt(json["battery_percent"]) { remoteBattery = "BAT \(battery)%" }
        if let signal = jsonInt(json["rssi"]) { rssi = "RSSI \(signal)" }
