...
final class AVOHardwareReceiver: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    @Published var cloudAPI = "https://live.avoperformance.org/api/latest"
    ...
    private func applyCloudPacket(_ json: [String: Any], rawText: String, httpCode: Int) {
        ...
        // /api/latest is the source of truth for vest health.
        // Do not mark the vest as connected blindly on every poll: the server may be
        // reporting FROZEN after 5s or DISCONNECTED after 15s without telemetry.
        if json["vestStatus"] != nil || json["connection"] != nil || json["proState"] != nil {
            parseJSON(json)
    ...
    private func applyServerVestStatusFromLatest(_ json: [String: Any]) {
        ...
        let frozen = jsonBool(vestStatus?["frozen"]) ?? jsonBool(connection?["dataFrozen"]) ?? jsonBool(proState?["dataFrozen"]) ?? state.contains("FROZEN")
        let disconnected = jsonBool(vestStatus?["disconnected"]) ?? jsonBool(connection?["disconnected"]) ?? jsonBool(proState?["vestDisconnected"]) ?? state.contains("DISCONNECTED")
        let connected = jsonBool(vestStatus?["connected"]) ?? jsonBool(proState?["vestConnected"]) ?? (!frozen && !disconnected && (state.contains("CONNECTED") || state.contains("ONLINE") || state == "OK"))
        let age = jsonDouble(vestStatus?["ageSeconds"]) ?? jsonDouble(proState?["lastTelemetryAgeSeconds"]) ?? jsonDouble(connection?["ageSeconds"]) ?? lastVestSeenSeconds
