...
final class AVOHardwareReceiver: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    @Published var cloudAPI = "http://79.117.33.132:5000/api/latest"
    ...
    private var lastRawPacketSignature = ""
    private var lastRawPacketChangeDate = Date.distantPast
    private var lastZoneInsideState: Bool?
    private var lastHorseNotification = ""
    private var lastRiderNotification = ""
    private var didNotifyFrozenData = false
    private var didNotifyNoData = false
    ...
    private func markVestDisconnected(reason: String) {
        ...
        let isFrozen = lower.contains("congel") || lower.contains("stale")
        let isNoData = lower.contains("sin datos") || lower.contains("no data")

        // Anti-spam: notify once for each frozen/no-data state until real packets arrive again.
        if isFrozen && didNotifyFrozenData {
            vestIsConnected = false
            vestConnectionState = "VEST DISCONNECTED · \(reason)"
            vestConnectionAlert = "Chaleco desconectado · \(reason)"
            return
        }
        if isNoData && didNotifyNoData {
            vestIsConnected = false
            vestConnectionState = "VEST DISCONNECTED · \(reason)"
            vestConnectionAlert = "Chaleco desconectado · \(reason)"
            return
        ...
        if isFrozen { didNotifyFrozenData = true }
        if isNoData { didNotifyNoData = true }

        if vestIsConnected || isFrozen || isNoData {
            vestConnectionAlert = "Chaleco desconectado · \(reason)"
            emitTrainingEvent(
                type: isFrozen ? "vest.frozen" : "vest.disconnected",
                title: isFrozen ? "Datos congelados" : "Chaleco desconectado",
                body: reason
            )
        }
        vestIsConnected = false
        ...
        connectionWatchTask = Task { [weak self] in
            while !Task.isCancelled {
                ...
                await MainActor.run {
                    ...
                    let staleAge = Date().timeIntervalSince(self.lastRawPacketChangeDate)
                    if self.vestIsConnected && (age > 4.0 || staleAge > 6.0) {
                        self.markVestDisconnected(reason: staleAge > 6.0 ? "datos congelados 6s" : "sin datos 4s")
                    }
                }
            }
        }
    ...
    private func emitTrainingEvent(type: String, title: String, body: String) {
        ...
        // Real iOS local push event: banner + sound + vibration.
        // Silent events such as loading horse/rider update the app feed only.
        if type != "horse.loaded" && type != "rider.loaded" {
    ...
    func saveTrainingZoneToServer(_ zone: TrainingZone, horseName: String) {
        ...
        let endpoint = cloudAPI.replacingOccurrences(of: "/api/latest", with: "/api/geofence")
        guard let url = URL(string: endpoint) else {
            geofenceStatus = "SERVER GEOFENCE BAD URL"
            return
        }
        Task { [weak self] in
            ...
            do {
                ...
                await MainActor.run {
                    self?.geofenceStatus = (200...299).contains(code) ? "GEOFENCE SAVED SERVER" : "SERVER GEOFENCE HTTP \(code)"
                    self?.emitTrainingEvent(type: "geofence.server", title: "Geocerca servidor", body: self?.geofenceStatus ?? "SERVER")
                }
            } catch {
                await MainActor.run {
                    self?.geofenceStatus = "SERVER GEOFENCE OFFLINE"
    ...
    private func fetchRaspberryLatest() async {
        ...
        guard let url = URL(string: cloudAPI) else {
            await MainActor.run {
        ...
        do {
            ...
            guard let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
                await MainActor.run {
                    self.cloudStatus = "CLOUD JSON ERROR"
                    self.cloudLastError = "INVALID JSON"
            ...
            await applyCloudPacket(obj, rawText: text, httpCode: code)
        } catch {
            await MainActor.run {
    ...
    func parsePacket(_ text: String) {
        ...
        if signature == lastRawPacketSignature {
            if Date().timeIntervalSince(lastRawPacketChangeDate) > 6.0 {
                packetStatus = "PACKETS STALE"
                markVestDisconnected(reason: "datos congelados 6s")
                return
            }
        } else {
            lastRawPacketSignature = signature
            lastRawPacketChangeDate = Date()
        }
