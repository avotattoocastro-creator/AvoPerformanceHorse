...
struct AVOHomeLauncherView: View {
    ...
    private let appVersionText = "VERSION 1.4.6"
    ...
    private func launchStatusEditable(geo: GeometryProxy, e: AVOVisualElement) -> some View {
        HStack(spacing: 0) {
            launchStatusTile(icon: "server.rack", title: "SERVER", value: launchServerValue, color: launchServerColor, geo: geo)
            launchStatusDivider
            launchStatusTile(icon: "figure.equestrian.sports", title: "VESTS ONLINE", value: launchVestValue, color: launchVestColor, geo: geo)
            launchStatusDivider
            launchStatusTile(icon: "house.lodge", title: "STABLE", value: hardware.activeVestHorse == "NO HORSE" ? "NO HORSE" : hardware.activeVestHorse, color: .green, geo: geo)
            launchStatusDivider
    ...
    private var launchVestValue: String {
        let server = hardware.serverVestState.uppercased()
        if server == "DISCONNECTED" || hardware.serverVestDisconnected { return "OFFLINE" }
        if server == "FROZEN" || hardware.serverVestFrozen { return "FROZEN" }
        if server == "CONNECTED" || hardware.serverVestConnected { return "1 CONNECTED" }
        let s = hardware.vestConnectionState.uppercased()
        if s.contains("FROZEN") { return "FROZEN" }
        if hardware.vestIsConnected { return "1 CONNECTED" }
    ...
    private var launchVestColor: Color {
        ...
        if v.contains("CONNECTED") { return .green }
    ...
    private func launchMenuBar(geo: GeometryProxy) -> some View {
        HStack(spacing: 5) {
            ...
            launchMenuButton(title: "TRAINING", icon: "waveform.path.ecg.rectangle", mode: .trainingReplay, geo: geo)
