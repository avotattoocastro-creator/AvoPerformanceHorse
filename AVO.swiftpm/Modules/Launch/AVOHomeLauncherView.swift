...
struct AVOHomeLauncherView: View {
    ...
    private let appVersionText = "VERSION 1.1.2"
    ...
    private func launchStatusEditable(geo: GeometryProxy, e: AVOVisualElement) -> some View {
        HStack(spacing: 0) {
            launchStatusTile(icon: "server.rack", title: "CONEXIÓN", value: "SERVIDOR", geo: geo)
            launchStatusDivider
            launchStatusTile(icon: "figure.equestrian.sports", title: "CHALECO", value: "CONECTADO", geo: geo)
            launchStatusDivider
            launchStatusTile(icon: "house.lodge", title: "CUADRA", value: "CONECTADA", geo: geo)
            launchStatusDivider
            launchStatusTile(icon: "antenna.radiowaves.left.and.right", title: "RTE", value: "CONECTADO", geo: geo)
        }
        .padding(.horizontal, 16)
        .background(Color.black.opacity(0.55))
        .clipShape(RoundedRectangle(cornerRadius: CGFloat(e.cornerRadius)))
    ...
    private func launchMenuBar(geo: GeometryProxy) -> some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 5) {
                ...
                launchMenuButton(title: "CONFIG HUB", icon: "slider.horizontal.3", mode: .configHub, geo: geo)
    ...
    private func launchUIStudioButton(geo: GeometryProxy) -> some View {
        let buttonWidth = max(54, min(72, geo.size.width * 0.047))
        return Button {
            showUIStudio = true
        } label: {
            VStack(spacing: 4) {
    ...
    private func launchStatusTile(icon: String, title: String, value: String, geo: GeometryProxy) -> some View {
        HStack(spacing: 12) {
            Image(systemName: icon)
