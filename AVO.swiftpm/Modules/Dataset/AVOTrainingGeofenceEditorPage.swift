...
private enum AVOTrainingGeofenceLibrary {
    private static let key = "AVOTrainingGeofenceLibraryV1"

    static func load() -> [String: [TrainingZone]] {
        guard let data = UserDefaults.standard.data(forKey: key),
              let decoded = try? JSONDecoder().decode([String: [TrainingZone]].self, from: data) else { return [:] }
        return decoded
    }

    static func zones(for horse: String) -> [TrainingZone] {
        load()[horse] ?? []
    }

    static func save(_ zone: TrainingZone, for horse: String) {
        var library = load()
        var zones = library[horse] ?? []
        zones.removeAll { $0.name == zone.name }
        zones.insert(zone, at: 0)
        if zones.count > 20 { zones.removeLast(zones.count - 20) }
        library[horse] = zones
...
struct AVOTrainingGeofenceEditorPage: View {
    ...
    private var draftZone: TrainingZone {
        TrainingZone(
            ...
            fixedCameraLatitudeDelta: cameraDeltaFromZoomMeters(),
            fixedCameraLongitudeDelta: cameraDeltaFromZoomMeters()
        )
    }
    ...
    private var controlPanel: some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    HStack {
                        Text("FIXED CAMERA ZOOM INSIDE GEOFENCE")
                            .font(.system(size: 9, weight: .black, design: .monospaced))
                            .foregroundStyle(.white.opacity(0.62))
                        Spacer()
                        Text("\(Int(fixedCameraZoomMeters)) m")
                            .font(.system(size: 13, weight: .black, design: .monospaced))
                            .foregroundStyle(.cyan)
                    }
                    Slider(value: $fixedCameraZoomMeters, in: 60...900, step: 10)
                }
                .padding(8)
                .background(Color.black.opacity(0.45))
                .clipShape(RoundedRectangle(cornerRadius: 10))
    ...
    private var savedZonesPanel: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("SAVED GEOFENCES FOR HORSE")
                .font(.system(size: 9, weight: .black, design: .monospaced))
                .foregroundStyle(.white.opacity(0.62))
            if savedZones.isEmpty {
                Text("No hay geocercas guardadas para \(activeHorseName)")
                    .font(.system(size: 10, weight: .bold, design: .monospaced))
                    .foregroundStyle(.orange)
            } else {
                ForEach(savedZones.prefix(6), id: \.name) { zone in
                    Button { applySavedZone(zone) } label: {
                        HStack {
                            Text(zone.name)
                            Spacer()
                            Text(zone.isFreeDrawZone ? "POLYGON" : "CIRCLE")
                            Text(String(format: "%.0fm", zone.radiusMeters))
                        }
                        .font(.system(size: 10, weight: .black, design: .monospaced))
                        .foregroundStyle(.green)
                        .padding(8)
    ...
    private var modeButtons: some View {
        VStack(spacing: 8) {
            ...
            Text("Con DRAW ON pintas la geocerca. En LIVE, al entrar se fija cámara/zoom de esta geocerca; al salir vuelve a seguimiento dinámico.")
    ...
    private func loadZone() {
        ...
        fixedCameraZoomMeters = max(60, min(900, settings.trainingZone.fixedCameraLatitudeDelta * 111_000))
        ...
        savedZones = AVOTrainingGeofenceLibrary.zones(for: activeHorseName)
    }

    private func applySavedZone(_ zone: TrainingZone) {
        zoneName = zone.name
        latitudeText = String(format: "%.6f", zone.latitude)
        longitudeText = String(format: "%.6f", zone.longitude)
        radiusMeters = zone.radiusMeters
        fixedCameraZoomMeters = max(60, min(900, zone.fixedCameraLatitudeDelta * 111_000))
    ...
    private func saveZoneOnly() {
        ...
        AVOTrainingGeofenceLibrary.save(draftZone, for: activeHorseName)
        savedZones = AVOTrainingGeofenceLibrary.zones(for: activeHorseName)
        hardware.geofenceStatus = "GEOFENCE SAVED APP"
        hardware.setActiveVestHorse(activeHorseName)
        hardware.updateTrainingZonePresence(settings.trainingZone)
        hardware.saveTrainingZoneToServer(draftZone, horseName: activeHorseName)
    }
    ...
    private func cameraDeltaFromZoomMeters() -> Double {
        let meters = max(60, min(900, fixedCameraZoomMeters))
        return max(0.00055, min(0.012, meters / 111_000.0))
    }
