...
struct AVOLiveTrainingDashboardPage: View {
    ...
    @State private var didDeliverFrozenNotification = false
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            didDeliverFrozenNotification = false
        }
        ...
        HStack(spacing: 10) {
            ...
            HStack(spacing: 8) {
                Image(systemName: "icloud.fill")
                    ...
                    .foregroundStyle(cloudConnectionText == "DISCONNECTED" ? .orange : .green)
                VStack(alignment: .leading, spacing: 2) {
                    Text("CLOUD")
                        .font(.system(size: 10, weight: .black, design: .monospaced))
    ...
    private func liveDashboardExact(geo: GeometryProxy) -> some View {
        VStack(spacing: 8) {
            HStack(spacing: 10) {
                ...
                .frame(width: min(max(geo.size.width * 0.54, 540), geo.size.width * 0.60))

                VStack(spacing: 8) {
                    liveMetricStack
                        .frame(height: max(220, min(310, geo.size.height * 0.29)))
                    Spacer(minLength: 0)
                    horseRiderGravityField
                        .frame(height: max(250, min(360, geo.size.height * 0.36)))
                }
    ...
    private var liveMapCard: some View {
        ...
        .overlay(RoundedRectangle(cornerRadius: 10).stroke(mapNeonColor.opacity(0.95), lineWidth: hardware.vestIsConnected ? 2.2 : 3.0))
        .shadow(color: mapNeonColor.opacity(0.38), radius: 12, x: 0, y: 0)
        .clipShape(RoundedRectangle(cornerRadius: 10))
    }
    ...
    private var liveHorseStatusRow: some View {
        HStack(spacing: 0) {
            ...
            liveStatusBlock(icon: "icloud.fill", title: "CONNECTION", value: connectionStatusText, color: hardware.vestIsConnected ? .green : .red)
            liveStatusBlock(icon: "location.fill", title: "GPS STATUS", value: gpsStatusText + "\n" + rtkAccuracyText, color: gpsStatusText.contains("FIX") || gpsStatusText.contains("TRACKING") ? .green : .orange)
        ...
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("NOVEDADES v1.1.1")
                    ...
                    .foregroundStyle(.cyan)
                Text("""
• Anti-spam datos congelados real
• Layout responsive iPad Pro
• Mapa neón por señal
• Cámara fija en geocerca
• Build 24 App Store
""")
    ...
    private var zoneMetricColor: Color {
        hardware.isInsideTrainingZone ? .green : .orange
    }

    private var mapNeonColor: Color {
        if !hardware.vestIsConnected { return .red }
    ...
    private var cloudConnectionText: String {
        hardware.vestIsConnected && hardware.cloudStatus.uppercased().contains("ONLINE") ? "CONNECTED" : "DISCONNECTED"
    }
    ...
    private func deliverLatestTrainingNotificationIfNeeded() {
        ...
        let title = hardware.latestNotificationTitle
        let body = hardware.latestNotificationBody
        let low = (title + " " + body).lowercased()
        let isFrozen = low.contains("congel") || low.contains("frozen") || low.contains("stale")

        if hardware.vestIsConnected && !isFrozen {
            didDeliverFrozenNotification = false
        }

        // Anti-spam real: los datos congelados solo se notifican una vez hasta que vuelva telemetría real.
        if isFrozen && didDeliverFrozenNotification {
        ...
        if isFrozen {
            didDeliverFrozenNotification = true
        }
