...
struct AVOLiveTrainingDashboardPage: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // v1.4.1 BUILD56: do NOT auto-sync sessions on dashboard open.
    ...
    private var liveBottomNavigation: some View {
        HStack(spacing: 0) {
            ...
            Button {
                selectedTab = "SESSION"
                selectedSessionHorseId = normalizedHorseId(activeHorseName)
                // v1.4.1 BUILD56: opening SESSION must stay local-only.
                loadLocalTrainingSessionsForSelectedHorse(status: "LOCAL CACHE · TAP REFRESH TO SYNC")
            } label: {
    ...
    private func loadRemoteTrainingSessions(force: Bool) {
        ...
        let horse = normalizedHorseId(selectedSessionHorseId)
    ...
    private func syncRemoteSessionsToLocalCache(horseId: String, force: Bool) {
        ...
        guard let url = URL(string: "https://live.avoperformance.org/api/sessions/index_light?horseId=\(encodedHorse)&limit=20") else {
            isSyncingRemoteSessions = false
            sessionsLoadStatus = "BAD SERVER URL"
            return
        }
        ...
        Task {
            do {
                ...
                guard (200...299).contains(code), !data.isEmpty else {
                    await MainActor.run {
                        isSyncingRemoteSessions = false
                        sessionsLoadStatus = "LOCAL ONLY · SERVER \(code)"
    ...
    private func appLocalRootURL() -> URL {
        if let root = stableStore.rootFolderURL {
            // v1.4.1 BUILD56: always use the configured iPad folder selected in SETTINGS > APP FOLDER.
    ...
    private func localSessionsVisiblePathLabel(horseId: String? = nil) -> String {
        ...
        if let horseId, !horseId.isEmpty {
            return "\(rootName)/Sessions/\(safeLocalName(normalizedHorseId(horseId), fallback: "HORSE_001"))/<session>"
        }
    ...
    private func ensureLocalSessionFolder(horseId: String, sessionId: String) -> URL? {
        ...
        let horse = safeLocalName(normalizedHorseId(horseId), fallback: "HORSE_001")
    ...
    private func downloadRemoteSessionDetailData(sessionId: String) async -> Data? {
        ...
        // v1.4.1 BUILD56: Raspberry exposes the detail endpoint as /api/session/<sessionId>.
        ...
        Task {
            if let data = await downloadRemoteSessionDetailData(sessionId: sessionId) {
                let fallback = remoteTrainingSessions.first(where: { $0.id == sessionId }) ?? AVORemoteTrainingSession(
                    id: sessionId,
                    horseId: normalizedHorseId(selectedSessionHorseId),
                    horseName: normalizedHorseId(selectedSessionHorseId),
                    status: "SAVED",
