...
struct AVOLiveTrainingDashboardPage: View {
    ...
    private func syncRemoteSessionsToLocalCache(horseId: String, force: Bool) {
        ...
        guard let url = URL(string: "https://live.avoperformance.org/api/sessions/index?horseId=\(encodedHorse)") else {
        ...
        Task {
            do {
                ...
                let serverSessions = parseRemoteSessionsList(json)
                ...
                for item in serverSessions {
                    ...
                    saveLocalSessionMeta(item)
                    if localSessionDetailExists(horseId: item.horseId, sessionId: sid), !force {
                        skipped += 1
                        continue
                    }
                    if let detailData = await downloadRemoteSessionDetailData(sessionId: sid) {
                        if saveLocalSessionDetailData(detailData, fallback: item) { downloaded += 1 }
                    } else if saveLocalSessionMeta(item) {
                        skipped += 1
                    }
                }
                ...
                await MainActor.run {
                    isSyncingRemoteSessions = false
                    loadLocalTrainingSessionsForSelectedHorse(status: downloaded > 0 ? "SYNCED \(downloaded) NEW" : (serverSessions.isEmpty ? "NO SERVER SESSIONS" : "LOCAL UP TO DATE"))
                    print("AVO LOCAL SESSIONS SYNC", "server=", serverSessions.count, "downloaded=", downloaded, "skipped=", skipped, "index=", savedIndex)
                }
            } catch {
                await MainActor.run {
                    isSyncingRemoteSessions = false
    ...
    private func downloadRemoteSessionDetailData(sessionId: String) async -> Data? {
        ...
        // v1.3.8 BUILD53: Raspberry exposes the detail endpoint as /api/session/<sessionId>.
        // Do not use /api/sessions/download?sessionId=... because that route does not exist on the stable server.
        let urlStrings = [
                    ...
                    ScrollView {
                        LazyVStack(spacing: 8) {
                            ...
                            if remoteSessionsForSelectedHorse.count > 60 {
                                Text("Showing 60 recent local sessions from \(remoteSessionsForSelectedHorse.count). Tap REFRESH to sync or filter by horse.")
                                    .font(.system(size: 10, weight: .black, design: .monospaced))
                                    .foregroundStyle(.white.opacity(0.52))
                                    .padding(.vertical, 8)
                            }
