...
struct AVOLiveTrainingDashboardPage: View {
    ...
    private func loadRemoteTrainingSessions(force: Bool) {
        ...
        guard let url = URL(string: "https://live.avoperformance.org/api/sessions/index") else { return }
        Task {
            do {
                var request = URLRequest(url: url)
                request.httpMethod = "GET"
                ...
                await MainActor.run {
                    remoteTrainingSessions = parsed.sorted { remoteSessionSortDate($0) > remoteSessionSortDate($1) }
            ...
            do {
                ...
                await MainActor.run {
                    if let idx = remoteTrainingSessions.firstIndex(where: { $0.id == detail.id || $0.id == sessionId }) {
                        remoteTrainingSessions[idx] = detail
                    } else {
                        remoteTrainingSessions.append(detail)
                    }
    ...
    private func parseRemoteSessionsList(_ json: Any) -> [AVORemoteTrainingSession] {
        ...
        if let nested = dict["session"] as? [String: Any] { return [parseRemoteSession(nested)].compactMap { $0 } }
        if let nested = dict["meta"] as? [String: Any] { return [parseRemoteSession(nested)].compactMap { $0 } }
    ...
    private func parseRemoteSession(_ dictAny: Any) -> AVORemoteTrainingSession? {
        ...
        if let nested = dict["session"] as? [String: Any] { dict = nested }
        if let nested = dict["meta"] as? [String: Any] { dict.merge(nested) { current, _ in current } }
        ...
        ProBox("SERVER TRAINING SESSIONS · HORSE INDEX") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                if remoteSessionsForSelectedHorse.isEmpty {
                    ...
                    ScrollView {
                        VStack(spacing: 8) {
                            ForEach(remoteSessionsForSelectedHorse) { session in
    ...
    private func remoteSessionRow(_ session: AVORemoteTrainingSession) -> some View {
        ...
        return Button {
            ...
            loadRemoteTrainingSessionDetail(session.id)
        } label: {
            HStack(spacing: 12) {
