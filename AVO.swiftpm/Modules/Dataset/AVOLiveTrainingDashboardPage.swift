...
struct AVOLiveTrainingDashboardPage: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // v1.4.0 BUILD55: do NOT auto-sync sessions on dashboard open.
    ...
    private var liveBottomNavigation: some View {
        HStack(spacing: 0) {
            ...
            Button {
                selectedTab = "SESSION"
                selectedSessionHorseId = normalizedHorseId(activeHorseName)
                // v1.4.0 BUILD55: opening SESSION must stay local-only.
                loadLocalTrainingSessionsForSelectedHorse(status: "LOCAL CACHE · TAP REFRESH TO SYNC")
            } label: {
    ...
    private func syncRemoteSessionsToLocalCache(horseId: String, force: Bool) {
        ...
        guard let url = URL(string: "https://live.avoperformance.org/api/sessions/index?horseId=\(encodedHorse)&limit=20&light=1") else {
    ...
    private func downloadRemoteSessionDetailData(sessionId: String) async -> Data? {
        ...
        // v1.4.0 BUILD55: Raspberry exposes the detail endpoint as /api/session/<sessionId>.
            ...
            VStack(alignment: .leading, spacing: 10) {
                if let session = selectedRemoteSession {
                    ...
                    VStack(spacing: 6) {
                        ...
                        MiniText(name: "SOURCE", value: "iPad local cache · Sessions/<horse>/<session>", color: .cyan)
                    }
                    Spacer()
                } else {
                    Spacer()
