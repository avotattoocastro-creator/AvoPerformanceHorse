...
struct AVOLiveTrainingDashboardPage: View {
    ...
    @State private var sessionsLoadStatus = "LOCAL SESSIONS WAITING"
    @State private var sessionsLocalStatus = "LOCAL CACHE READY"
    @State private var sessionsLastFetchDate = Date.distantPast
    @State private var isSyncingRemoteSessions = false
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // v1.4.4 BUILD59: do NOT auto-sync sessions on dashboard open.
    ...
    private var connectionBanner: some View {
        HStack(spacing: 10) {
            ...
            Text(hardware.remoteBattery.isEmpty ? "BAT --" : hardware.remoteBattery)
                .font(.system(size: 12, weight: .black, design: .monospaced))
                .foregroundStyle(batteryColor)
                .padding(.horizontal, 8)
                .frame(height: 20)
                .background(batteryColor.opacity(0.10))
                .overlay(RoundedRectangle(cornerRadius: 6).stroke(batteryColor.opacity(0.34), lineWidth: 1))
                .clipShape(RoundedRectangle(cornerRadius: 6))
            Spacer()
            Text("ACTIVE VEST: \(activeVestDisplay) • HORSE: \(activeHorseName) • RIDER: \(activeRiderDisplay)")
                .font(.system(size: 12, weight: .black, design: .monospaced))
    ...
    private var liveHorseStatusRow: some View {
        // Technical GPS / RTK / geofence strip. Keep this row to 6 cards so the existing dashboard layout does not move.
        HStack(spacing: 0) {
            liveStatusBlock(icon: "location.fill", title: "GPS DATA", value: gpsDataText, color: mapSignalColor)
            liveStatusBlock(icon: "scope", title: "GPS STATUS", value: rtkFixLabel, color: rtkSignalColor)
            liveStatusBlock(icon: "antenna.radiowaves.left.and.right", title: "NTRIP", value: hardware.gpsNTRIP ? "ON" : "OFF", color: hardware.gpsNTRIP ? .green : .red)
            liveStatusBlock(icon: "waveform.path.ecg", title: "RTCM PACKETS", value: rtcmPacketsText, color: hardware.gpsRtcmBytes > 0 ? .green : .orange)
            ...
            liveStatusBlock(icon: "server.rack", title: "RTK SERVER", value: rtkServerText, color: rtkServerColor)
        }
    ...
    private var liveTrainingLoadRow: some View {
        // Wide professional training-load panel: gauge + 3D live surface + compact biomech stats.
        // Kept in the same marked dashboard zone so the existing page order does not move.
        HStack(spacing: 8) {
            trainingLoadGauge
                .frame(width: 190)

            AVOTrainingLoadSurfacePanel(
                title: "TRAINING LOAD SURFACE 3D",
                speedValues: hardware.speedHistory,
                impactValues: hardware.impactHistory,
                symmetryProgress: hardware.impactHistory.isEmpty ? 0.0 : (Double(syncIndexText.replacingOccurrences(of: "%", with: "")) ?? 0) / 100.0,
                impactG: hardware.imuImpact,
                sessionTime: liveSessionDurationText
            )
            .frame(maxWidth: .infinity, maxHeight: .infinity)

            AVOTrainingLoadSideStats(
                symmetryText: hardware.impactHistory.isEmpty ? "NO DATA" : syncIndexText,
                symmetrySubtitle: hardware.impactHistory.isEmpty ? "NO REAL SENSOR" : syncQualityText,
                symmetryColor: hardware.impactHistory.isEmpty ? .orange : syncColor,
                impactText: hardware.impactHistory.isEmpty ? "NO DATA" : String(format: "%.1f G", hardware.imuImpact),
                impactSubtitle: hardware.impactHistory.isEmpty ? "WAITING" : impactQualityText,
                impactColor: hardware.impactHistory.isEmpty ? .orange : impactQualityColor,
                progress: hardware.impactHistory.isEmpty ? 0.0 : (Double(syncIndexText.replacingOccurrences(of: "%", with: "")) ?? 0) / 100.0
            )
            .frame(width: 130)
        }
    }
    ...
    private var liveBottomNavigation: some View {
        HStack(spacing: 0) {
            ...
            Button {
                selectedTab = "SESSION"
                selectedSessionHorseId = canonicalSessionHorseId("HORSE_001")
                // v1.4.4 BUILD59: opening SESSION must stay local-only.
                loadLocalTrainingSessionsForSelectedHorse(status: "LOCAL CACHE · TAP REFRESH TO SYNC")
            } label: {
    ...
    private var vestBannerText: String {
        ...
        if vestIsServerDisconnected { return "Vest disconnected by CLOUD" }
        if vestIsServerFrozen { return "Vest data frozen by CLOUD" }
        if vestIsServerConnected { return "Vest connected by CLOUD" }
        return hardware.vestConnectionAlert
    }
    private var activeVestDisplay: String {
        if vestIsServerDisconnected { return "NONE" }
        let id = hardware.selectedVestID.trimmingCharacters(in: .whitespacesAndNewlines)
        return id.isEmpty ? "UNKNOWN" : id
    }

    private var activeRiderDisplay: String {
        let rider = hardware.activeVestRider.trimmingCharacters(in: .whitespacesAndNewlines)
        return rider.isEmpty ? "NO RIDER" : rider
    }
    ...
    private var gpsDataText: String {
        "SAT \(hardware.gpsSatellites)\n\(rtkAccuracyText)"
    }

    private var rtcmPacketsText: String {
        let value = max(0, hardware.gpsRtcmBytes)
        if value >= 1_000_000 { return String(format: "%.1fM", Double(value) / 1_000_000.0) }
        if value >= 10_000 { return String(format: "%.1fk", Double(value) / 1_000.0) }
        return "\(value)"
    }

    private var rtkServerText: String {
        if vestIsServerDisconnected { return "NO" }
        if hardware.rtkProxyConnected { return "OK" }
        let status = hardware.rtkProxyStatus.uppercased()
        if status.contains("STREAM") || status.contains("OK") || status.contains("CONNECTED") { return "OK" }
        if hardware.gpsNTRIP && hardware.gpsRtcmBytes > 0 { return "OK" }
        return "NO"
    }

    private var rtkServerColor: Color {
        rtkServerText == "OK" ? .green : .red
    }
    ...
    private var remoteHorseIds: [String] {
        ...
        for session in remoteTrainingSessions {
            let id = canonicalSessionHorseId(session.horseId)
    ...
    private var remoteSessionsForSelectedHorse: [AVORemoteTrainingSession] {
        remoteTrainingSessions
            .filter { canonicalSessionHorseId($0.horseId) == canonicalSessionHorseId(selectedSessionHorseId) }
    ...
    // v1.4.4 BUILD59: storage and Raspberry sync are keyed by the stable server horseId.
    ...
    private func canonicalSessionHorseId(_ value: String? = nil) -> String {
        let raw = (value ?? selectedSessionHorseId).trimmingCharacters(in: .whitespacesAndNewlines)
        let upper = raw.uppercased().replacingOccurrences(of: " ", with: "_")
        if upper.isEmpty || upper == "NO_HORSE" || upper == "NO_HORSE_SELECTED" || upper == "LEZAMA_DREAMS" || upper == "LEZAMA_DREAMS_" {
            return "HORSE_001"
        }
        if upper.hasPrefix("HORSE_") { return upper }
        return "HORSE_001"
    }

    private var activeHorseDisplayName: String {
        let name = activeHorseName.trimmingCharacters(in: .whitespacesAndNewlines)
        return name.isEmpty || name.uppercased().hasPrefix("HORSE_") ? "Lezama dreams" : name
    }

    private func loadRemoteTrainingSessions(force: Bool) {
        // SESSION PAGE STRATEGY:
        // 1) Open fast from iPad local cache.
        // 2) In background, ask Raspberry for the lightweight index.
        // 3) Download only missing/new session JSON files into the app folder.
        // 4) Refresh the UI from local files, not by streaming heavy online data.
        guard force || Date().timeIntervalSince(sessionsLastFetchDate) > 10.0 else { return }
        sessionsLastFetchDate = Date()
        let horse = canonicalSessionHorseId(selectedSessionHorseId)
        loadLocalTrainingSessionsForSelectedHorse(status: "LOCAL CACHE")
        syncRemoteSessionsToLocalCache(horseId: horse, force: force)
    }

    private func loadLocalTrainingSessionsForSelectedHorse(status: String? = nil) {
        let horse = canonicalSessionHorseId(selectedSessionHorseId)
        let local = loadLocalCachedSessions(horseId: horse).map { lightweightSessionForList($0) }
        remoteTrainingSessions = local.sorted { remoteSessionSortDate($0) > remoteSessionSortDate($1) }
        if selectedRemoteSessionId == nil || !remoteTrainingSessions.contains(where: { $0.id == selectedRemoteSessionId }) {
            selectedRemoteSessionId = remoteSessionsForSelectedHorse.first?.id
        }
        if let status {
            sessionsLoadStatus = local.isEmpty ? "LOCAL EMPTY" : status
        }
        sessionsLocalStatus = "LOCAL \(local.count)"
    }

    private func syncRemoteSessionsToLocalCache(horseId: String, force: Bool) {
        guard !isSyncingRemoteSessions else { return }
        isSyncingRemoteSessions = true
        sessionsLoadStatus = "SYNCING SERVER INDEX"

        let encodedHorse = horseId.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "HORSE_001"
        let urlStrings = [
            "https://live.avoperformance.org/api/sessions/index_light?horseId=\(encodedHorse)&limit=20",
            "https://live.avoperformance.org/api/sessions/index?horseId=\(encodedHorse)&limit=20&light=1"
        ]
        ...
        Task {
            do {
                ...
                for urlString in urlStrings {
                    ...
                    request.cachePolicy = .reloadIgnoringLocalCacheData
                    request.setValue("application/json", forHTTPHeaderField: "Accept")
                    let (tryData, response) = try await URLSession.shared.data(for: request)
                    let code = (response as? HTTPURLResponse)?.statusCode ?? 0
                    lastCode = code
                    if (200...299).contains(code), !tryData.isEmpty {
                        data = tryData
                        break
                    }
                }
                guard let data else {
                    await MainActor.run {
                        isSyncingRemoteSessions = false
                        sessionsLoadStatus = "LOCAL ONLY · SERVER \(lastCode)"
                        loadLocalTrainingSessionsForSelectedHorse(status: "LOCAL ONLY")
                    }
                    return
                }

                let json = try JSONSerialization.jsonObject(with: data)
                // v1.4.4 BUILD59 STABILITY FIX:
                ...
                // only when the user taps a specific session to open it.
                let allServerSessions = parseRemoteSessionsList(json)
                let serverSessions = Array(allServerSessions
                    .sorted { remoteSessionSortDate($0) > remoteSessionSortDate($1) }
                    .prefix(20))
                let savedIndex = saveServerSessionsIndexJSON(data, horseId: horseId)
                var savedMeta = 0

                for item in serverSessions {
                    let sid = item.id.trimmingCharacters(in: .whitespacesAndNewlines)
                    guard !sid.isEmpty else { continue }
                    if saveLocalSessionMeta(item) { savedMeta += 1 }
                }

                await MainActor.run {
                    isSyncingRemoteSessions = false
                    loadLocalTrainingSessionsForSelectedHorse(status: serverSessions.isEmpty ? "NO SERVER SESSIONS" : "INDEX SYNCED \(savedMeta)")
                    print("AVO LOCAL SESSIONS INDEX SYNC", "server=", allServerSessions.count, "cachedMeta=", savedMeta, "downloadedDetails=0", "index=", savedIndex)
                }
            } catch {
                await MainActor.run {
                    isSyncingRemoteSessions = false
                    sessionsLoadStatus = "LOCAL ONLY · SERVER ERROR"
                    loadLocalTrainingSessionsForSelectedHorse(status: "LOCAL ONLY")
                    print("AVO LOCAL SESSIONS SYNC ERROR", error)
                }
            }
        }
    }
    ...
    private func appLocalRootURL() -> URL {
        if let root = stableStore.rootFolderURL {
            // v1.4.4 BUILD59: always use the configured iPad folder selected in SETTINGS > APP FOLDER.
        ...
        let fallback = docs.appendingPathComponent("AVO_Horse_App", isDirectory: true)
        try? FileManager.default.createDirectory(at: fallback, withIntermediateDirectories: true)
        return fallback
    }

    private func localSessionsRootURL() -> URL {
        let root = appLocalRootURL().appendingPathComponent("Sessions", isDirectory: true)
        try? FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        migrateLegacyLezamaSessionsIfNeeded(sessionsRoot: root)
        return root
    }

    private func migrateLegacyLezamaSessionsIfNeeded(sessionsRoot: URL) {
        let fm = FileManager.default
        let legacy = sessionsRoot.appendingPathComponent("LEZAMA_DREAMS", isDirectory: true)
        let canonical = sessionsRoot.appendingPathComponent("HORSE_001", isDirectory: true)
        guard fm.fileExists(atPath: legacy.path) else { return }
        try? fm.createDirectory(at: canonical, withIntermediateDirectories: true)
        guard let folders = try? fm.contentsOfDirectory(at: legacy, includingPropertiesForKeys: nil, options: [.skipsHiddenFiles]) else { return }
        for source in folders where source.hasDirectoryPath {
            let target = canonical.appendingPathComponent(source.lastPathComponent, isDirectory: true)
            if fm.fileExists(atPath: target.path) { continue }
            do {
                try fm.copyItem(at: source, to: target)
            } catch {
                print("AVO SESSION LEGACY MIGRATION COPY ERROR", source.lastPathComponent, error)
            }
        }
    }

    private func localSessionsVisiblePathLabel(horseId: String? = nil) -> String {
        let rootName = appLocalRootURL().lastPathComponent
        if let horseId, !horseId.isEmpty {
            return "\(rootName)/Sessions/\(safeLocalName(canonicalSessionHorseId(horseId), fallback: "HORSE_001"))/<session>"
        }
        return "\(rootName)/Sessions/<horse>/<session>"
    }

    private func writeSessionSyncReadmeIfNeeded() {
        let root = localSessionsRootURL()
        let readme = root.appendingPathComponent("README_SESSION_SYNC.txt")
        if FileManager.default.fileExists(atPath: readme.path) { return }
        let text = """
        AVO Performance Horse - Local Session Sync

        Raspberry sessions are cached here by the iPad app.
        Structure:
        Sessions/<HORSE_ID>/<SESSION_ID>/index.json
        Sessions/<HORSE_ID>/<SESSION_ID>/session.json when a session is opened.

        This folder is intentionally local-first so visual analysis does not stream heavy server files.
        """
        try? text.write(to: readme, atomically: true, encoding: .utf8)
    }

    private func safeLocalName(_ raw: String, fallback: String) -> String {
        let allowed = CharacterSet.alphanumerics.union(CharacterSet(charactersIn: "_-"))
        let mapped = raw.uppercased().map { ch -> Character in
            let s = String(ch)
            return s.rangeOfCharacter(from: allowed) == nil ? "_" : ch
        }
        let out = String(mapped).trimmingCharacters(in: CharacterSet(charactersIn: "_"))
        return out.isEmpty ? fallback : out
    }

    private func ensureLocalSessionFolder(horseId: String, sessionId: String) -> URL? {
        let fm = FileManager.default
        let horse = safeLocalName(canonicalSessionHorseId(horseId), fallback: "HORSE_001")
        let sid = safeLocalName(sessionId, fallback: "SESSION_UNKNOWN")
        let folder = localSessionsRootURL().appendingPathComponent(horse, isDirectory: true).appendingPathComponent(sid, isDirectory: true)
        do {
            writeSessionSyncReadmeIfNeeded()
            try fm.createDirectory(at: folder, withIntermediateDirectories: true)
            return folder
        } catch {
            print("AVO LOCAL SESSION FOLDER ERROR", error)
            return nil
        }
    }

    private func localSessionDetailURL(horseId: String, sessionId: String) -> URL? {
        ensureLocalSessionFolder(horseId: horseId, sessionId: sessionId)?.appendingPathComponent("session.json")
    }

    private func localSessionMetaURL(horseId: String, sessionId: String) -> URL? {
        ensureLocalSessionFolder(horseId: horseId, sessionId: sessionId)?.appendingPathComponent("index.json")
    }

    private func localSessionDetailExists(horseId: String, sessionId: String) -> Bool {
        guard let url = localSessionDetailURL(horseId: horseId, sessionId: sessionId) else { return false }
        return FileManager.default.fileExists(atPath: url.path)
    }

    private func saveServerSessionsIndexJSON(_ data: Data, horseId: String) -> Bool {
        let horse = safeLocalName(canonicalSessionHorseId(horseId), fallback: "HORSE_001")
        let folder = localSessionsRootURL().appendingPathComponent(horse, isDirectory: true)
        do {
            writeSessionSyncReadmeIfNeeded()
            try FileManager.default.createDirectory(at: folder, withIntermediateDirectories: true)
            try data.write(to: folder.appendingPathComponent("server_index.json"), options: [.atomic])
            return true
        } catch {
            print("AVO SERVER INDEX SAVE ERROR", error)
            return false
        }
    }

    @discardableResult
    private func saveLocalSessionMeta(_ session: AVORemoteTrainingSession) -> Bool {
        guard let url = localSessionMetaURL(horseId: session.horseId, sessionId: session.id) else { return false }
        do {
            let data = try JSONSerialization.data(withJSONObject: localSessionJSONDictionary(session, lightweight: true), options: [.prettyPrinted, .sortedKeys])
            try data.write(to: url, options: [.atomic])
            // Also save a human-recognizable metadata file in the configured Files.app folder.
            let metaAlias = url.deletingLastPathComponent().appendingPathComponent("session_meta.json")
            try? data.write(to: metaAlias, options: [.atomic])
            return true
        } catch {
            print("AVO LOCAL SESSION META SAVE ERROR", error)
            return false
        }
    }

    private func saveLocalSessionDetailData(_ data: Data, fallback: AVORemoteTrainingSession) -> Bool {
        let parsedJSON = (try? JSONSerialization.jsonObject(with: data))
        let parsed = parsedJSON.flatMap { parseRemoteSessionsList($0).first ?? parseRemoteSession($0) } ?? fallback
        guard let folder = ensureLocalSessionFolder(horseId: parsed.horseId, sessionId: parsed.id) else { return false }
        do {
            try data.write(to: folder.appendingPathComponent("session.json"), options: [.atomic])
            saveLocalSessionMeta(parsed)
            return true
        } catch {
            print("AVO LOCAL SESSION DETAIL SAVE ERROR", error)
            return false
        }
    }

    private func localSessionJSONDictionary(_ session: AVORemoteTrainingSession, lightweight: Bool) -> [String: Any] {
        var dict: [String: Any] = [
            "id": session.id,
            "sessionId": session.id,
            "horseId": session.horseId,
            "horseName": session.horseName.isEmpty || session.horseName.uppercased().hasPrefix("HORSE_") ? activeHorseDisplayName : session.horseName,
            ...
            "localCachedAt": ISO8601DateFormatter().string(from: Date()),
            "source": "AVO_LOCAL_SESSION_CACHE"
        ]
        if !lightweight {
            dict["track"] = session.track.map { ["lat": $0.latitude, "lon": $0.longitude] }
            dict["samples"] = session.samples.map { sample in
                [
                    "time": sample.time,
                    "lat": sample.latitude as Any,
                    "lon": sample.longitude as Any,
                    "speedKmh": sample.speedKmh,
                    "impactG": sample.impactG,
    ...
    private func loadLocalCachedSessions(horseId: String) -> [AVORemoteTrainingSession] {
        let horse = safeLocalName(canonicalSessionHorseId(horseId), fallback: "HORSE_001")
        let horseFolder = localSessionsRootURL().appendingPathComponent(horse, isDirectory: true)
        guard let folders = try? FileManager.default.contentsOfDirectory(at: horseFolder, includingPropertiesForKeys: nil, options: [.skipsHiddenFiles]) else { return [] }
        var result: [AVORemoteTrainingSession] = []
        for folder in folders where folder.hasDirectoryPath {
            let detail = folder.appendingPathComponent("session.json")
            let meta = folder.appendingPathComponent("index.json")
            let url = FileManager.default.fileExists(atPath: detail.path) ? detail : meta
            guard FileManager.default.fileExists(atPath: url.path), let data = try? Data(contentsOf: url), let json = try? JSONSerialization.jsonObject(with: data) else { continue }
            if let parsed = parseRemoteSessionsList(json).first ?? parseRemoteSession(json) {
                result.append(parsed)
            }
        }
        return result.sorted { remoteSessionSortDate($0) > remoteSessionSortDate($1) }
    }

    private func loadLocalSessionDetail(session: AVORemoteTrainingSession) -> AVORemoteTrainingSession? {
        guard let url = localSessionDetailURL(horseId: session.horseId, sessionId: session.id),
              FileManager.default.fileExists(atPath: url.path),
              let data = try? Data(contentsOf: url),
              let json = try? JSONSerialization.jsonObject(with: data) else { return nil }
        return parseRemoteSessionsList(json).first ?? parseRemoteSession(json)
    }
    ...
    private func downloadRemoteSessionDetailData(sessionId: String) async -> Data? {
        ...
        // v1.4.4 BUILD59: Raspberry exposes the detail endpoint as /api/session/<sessionId>.
        // Details are downloaded one-at-a-time only when the user opens a session.
        // Do not bulk-download on page open.
        let urlStrings = [
            "https://live.avoperformance.org/api/session_light/\(safe)",
        ...
        for urlString in urlStrings {
            ...
            do {
                ...
                let (data, response) = try await URLSession.shared.data(for: request)
                let code = (response as? HTTPURLResponse)?.statusCode ?? 0
                if (200...299).contains(code), !data.isEmpty { return data }
            } catch {
                print("AVO LOCAL SESSION DETAIL DOWNLOAD TRY ERROR", urlString, error)
            }
        }
        return nil
    }
    ...
    private func openRemoteSessionAnalysis(_ session: AVORemoteTrainingSession) {
        ...
        // Prefer iPad local cache. Heavy sessions must open locally so the dashboard does not depend on streaming.
        if let localDetail = loadLocalSessionDetail(session: session) {
            analysisLoadStatus = "LOCAL ANALYSIS READY"
            analysisSessionToOpen = optimizedSessionForAnalysis(localDetail)
            return
        }

        // If the list item already has detail, save it locally and open it.
        if !session.samples.isEmpty || session.track.count > 20 {
            saveLocalSessionMeta(session)
            analysisLoadStatus = "LOCAL ANALYSIS READY"
            analysisSessionToOpen = optimizedSessionForAnalysis(session)
            return
        }

        // Last-resort background fetch: download once, save to iPad, then open from local cache.
        isLoadingSessionAnalysis = true
        analysisLoadStatus = "DOWNLOADING LOCAL COPY"
        Task {
            var opened: AVORemoteTrainingSession? = nil
            if let data = await downloadRemoteSessionDetailData(sessionId: session.id) {
                _ = saveLocalSessionDetailData(data, fallback: session)
                opened = loadLocalSessionDetail(session: session)
            }
            await MainActor.run {
                isLoadingSessionAnalysis = false
                if let opened {
                    if let idx = remoteTrainingSessions.firstIndex(where: { $0.id == opened.id || $0.id == session.id }) {
                        remoteTrainingSessions[idx] = lightweightSessionForList(opened)
                    }
                    selectedRemoteSessionId = opened.id
                    analysisLoadStatus = "LOCAL ANALYSIS READY"
                    analysisSessionToOpen = optimizedSessionForAnalysis(opened)
                } else {
                    analysisLoadStatus = "LOCAL INDEX ONLY"
                    analysisSessionToOpen = optimizedSessionForAnalysis(session)
                }
            }
        }
    ...
    private func loadRemoteTrainingSessionDetail(_ sessionId: String) {
        // Kept for compatibility with older buttons. It now downloads once into the local iPad cache.
        guard !sessionId.isEmpty else { return }
        Task {
            if let data = await downloadRemoteSessionDetailData(sessionId: sessionId) {
                let fallback = remoteTrainingSessions.first(where: { $0.id == sessionId }) ?? AVORemoteTrainingSession(
                    id: sessionId,
                    horseId: canonicalSessionHorseId(selectedSessionHorseId),
                    horseName: activeHorseDisplayName,
                    status: "SAVED",
                    startedAt: dateStringFromSessionId(sessionId),
                    endedAt: "",
                    durationSeconds: 0,
                    distanceKm: 0,
                    avgSpeedKmh: 0,
                ...
                _ = saveLocalSessionDetailData(data, fallback: fallback)
                await MainActor.run { loadLocalTrainingSessionsForSelectedHorse(status: "LOCAL DETAIL SAVED") }
            }
        }
    }
    ...
    private func parseRemoteSession(_ dictAny: Any) -> AVORemoteTrainingSession? {
        ...
        let rawHorseId = remoteString(dict, ["horseId", "horse_id", "activeHorse"]) ?? selectedSessionHorseId
        let horseId = canonicalSessionHorseId(rawHorseId)
        let horseName = remoteString(dict, ["horseName", "horse_name", "horse"]) ?? activeHorseDisplayName
    ...
    private var completedTrainingColumn: some View {
        ProBox("LOCAL TRAINING SESSIONS · SERVER SYNC") {
            VStack(alignment: .leading, spacing: 10) {
                HStack(spacing: 8) {
                    ...
                    metricCard("STATUS", sessionsLoadStatus, (sessionsLoadStatus.contains("LOCAL") || sessionsLoadStatus.contains("SYNCED") || sessionsLoadStatus.contains("UP TO DATE")) ? .green : .orange)
                ...
                if remoteSessionsForSelectedHorse.isEmpty {
                    VStack(spacing: 10) {
                        ...
                        Text("NO LOCAL SESSIONS")
                            .font(.system(size: 24, weight: .black, design: .monospaced))
                            .foregroundStyle(.white.opacity(0.72))
                        Text("Open this page to sync new Raspberry sessions into the iPad app folder. Local path: Sessions/\(selectedSessionHorseId)/<sessionId>.")
                    ...
                    ScrollView {
                        LazyVStack(spacing: 8) {
                            ForEach(Array(remoteSessionsForSelectedHorse.prefix(30))) { session in
                                remoteSessionRow(session)
                            }
                            if remoteSessionsForSelectedHorse.count > 30 {
                                Text("Showing 30 recent local sessions from \(remoteSessionsForSelectedHorse.count). Tap REFRESH to sync or filter by horse.")
                                    .font(.system(size: 10, weight: .black, design: .monospaced))
                                    .foregroundStyle(.white.opacity(0.52))
                                    .padding(.vertical, 8)
                            }
        ...
        .onAppear { loadLocalTrainingSessionsForSelectedHorse(status: "LOCAL CACHE · TAP REFRESH TO SYNC") }
    }

    private var completedDetailColumn: some View {
        ProBox("TRAINING DETAIL · LOCAL SESSION") {
            VStack(alignment: .leading, spacing: 10) {
                if let session = selectedRemoteSession {
                    ...
                    Button {
                        ...
                        HStack {
                            ...
                            Text("OPEN LOCAL SESSION ANALYSIS")
                            Spacer()
                            Text(isLoadingSessionAnalysis ? "PREPARING LOCAL COPY..." : analysisLoadStatus)
                        }
                        .font(.system(size: 12, weight: .black, design: .monospaced))
                    ...
                    VStack(spacing: 6) {
                        ...
                        MiniText(name: "SOURCE", value: localSessionsVisiblePathLabel(horseId: session.horseId), color: .cyan)
                    }
                    Spacer()
                } else {
                    Spacer()
                    Text("SELECT A LOCAL TRAINING")
                        .font(.system(size: 20, weight: .black, design: .monospaced))
                        .foregroundStyle(.white.opacity(0.72))
                    Text("Closed sessions are synced from the Raspberry into the iPad app folder, then opened locally.")
                        .font(.system(size: 12, weight: .bold, design: .monospaced))
                        .foregroundStyle(.white.opacity(0.52))
                    Spacer()
                }
...
private struct AVOTrainingLoadSurfacePanel: View {
    let title: String
    let speedValues: [Double]
    let impactValues: [Double]
    let symmetryProgress: Double
    let impactG: Double
    let sessionTime: String

    @State private var selectedMode: AVOTrainingSurfaceMode = .load
    @State private var selectedIndex: Int?
    @State private var isFrozen = false
    @State private var zoom: CGFloat = 1.0

    private var surfaceValues: [Double] {
        let raw: [Double]
        switch selectedMode {
        case .load:
            raw = !speedValues.isEmpty ? speedValues : impactValues
        case .impact:
            raw = !impactValues.isEmpty ? impactValues : speedValues
        case .symmetry:
            let base = max(0.05, min(1.0, symmetryProgress))
            raw = Array(repeating: base, count: max(18, max(speedValues.count, impactValues.count)))
        case .view3D:
            raw = !impactValues.isEmpty ? zip(speedValues.suffix(40), impactValues.suffix(40)).map { max(0.0, ($0.0 * 0.55) + ($0.1 * 1.35)) } : speedValues
        }
        let values = raw.suffix(44).map { max(0.0, $0) }
        guard values.count > 2 else {
            return [0.24, 0.32, 0.41, 0.36, 0.58, 0.43, 0.62, 0.51, 0.70, 0.46, 0.55, 0.38, 0.49, 0.64, 0.42, 0.30]
        }
        let maxValue = max(values.max() ?? 1.0, 0.001)
        return values.map { min(max($0 / maxValue, 0.07), 1.0) }
    }

    private var impactNormalized: [Double] {
        let values = impactValues.suffix(44).map { max(0.0, $0) }
        guard !values.isEmpty else { return surfaceValues.map { $0 * 0.35 } }
        let maxValue = max(values.max() ?? 1.0, 0.001)
        return values.map { min(max($0 / maxValue, 0.02), 1.0) }
    }

    private var statusColor: Color {
        if impactG >= 6.0 { return .red }
        if impactG >= 3.0 { return .orange }
        if symmetryProgress > 0.0 && symmetryProgress < 0.78 { return .orange }
        return .cyan
    }
    ...
    var body: some View {
        VStack(spacing: 6) {
            ...
            ZStack(alignment: .topLeading) {
                AVOTrainingLoadSurface(
                    values: surfaceValues,
                    impacts: impactNormalized,
                    symmetryProgress: symmetryProgress,
                ...
                if let selectedIndex {
                    surfaceTooltip(index: selectedIndex)
                        .padding(8)
                        .transition(.opacity.combined(with: .scale(scale: 0.96)))
                }

                HStack(spacing: 5) {
                    Text(isFrozen ? "FRAME FROZEN" : "LIVE SURFACE")
                        .font(.system(size: 9, weight: .black, design: .monospaced))
                        .foregroundStyle(isFrozen ? .orange : .green)
                    Circle()
                        .fill(isFrozen ? Color.orange : Color.green)
                        .frame(width: 6, height: 6)
                }
                .padding(.horizontal, 8)
                .padding(.vertical, 5)
                .background(Color.black.opacity(0.56))
                .overlay(RoundedRectangle(cornerRadius: 6).stroke((isFrozen ? Color.orange : Color.green).opacity(0.35), lineWidth: 1))
                .clipShape(RoundedRectangle(cornerRadius: 6))
                .padding(8)
            ...
            .gesture(
                DragGesture(minimumDistance: 0)
                    .onChanged { value in
                        let count = max(surfaceValues.count, 1)
                        let width = max(value.startLocation.x + value.translation.width, 0)
                        let approxWidth: CGFloat = 420
                        let fraction = min(max(width / approxWidth, 0), 1)
                        selectedIndex = min(count - 1, max(0, Int(round(fraction * CGFloat(count - 1)))))
                    }
            )
            .simultaneousGesture(
                MagnificationGesture()
                    .onChanged { value in zoom = min(max(value, 0.72), 1.85) }
            )
        ...
        .background(
            ZStack {
                LinearGradient(colors: [Color.black.opacity(0.64), Color.cyan.opacity(0.09), Color.green.opacity(0.05), Color.black.opacity(0.44)], startPoint: .topLeading, endPoint: .bottomTrailing)
                RadialGradient(colors: [statusColor.opacity(0.18), .clear], center: .center, startRadius: 8, endRadius: 260)
            }
        )
        .overlay(RoundedRectangle(cornerRadius: 10).stroke(statusColor.opacity(0.30), lineWidth: 1))
        .shadow(color: statusColor.opacity(0.18), radius: 12, x: 0, y: 0)
        .clipShape(RoundedRectangle(cornerRadius: 10))
    }

    private func modeButton(_ mode: AVOTrainingSurfaceMode) -> some View {
        Button {
            selectedMode = mode
            selectedIndex = nil
        } label: {
            Text(mode.title)
                .font(.system(size: 9, weight: .black, design: .monospaced))
                .foregroundStyle(selectedMode == mode ? .black : mode.color)
                .padding(.horizontal, 7)
                .padding(.vertical, 5)
                .background(selectedMode == mode ? mode.color.opacity(0.92) : Color.black.opacity(0.50))
                .overlay(RoundedRectangle(cornerRadius: 6).stroke(mode.color.opacity(0.38), lineWidth: 1))
                .clipShape(RoundedRectangle(cornerRadius: 6))
        }
        .buttonStyle(.plain)
    }

    private func surfaceTooltip(index: Int) -> some View {
        let safeIndex = min(max(index, 0), max(surfaceValues.count - 1, 0))
        let load = surfaceValues.isEmpty ? 0.0 : surfaceValues[safeIndex]
        let impact = impactNormalized.isEmpty ? 0.0 : impactNormalized[min(safeIndex, impactNormalized.count - 1)]
        return VStack(alignment: .leading, spacing: 3) {
            Text("T+") + Text("\(safeIndex)s")
            Text(String(format: "LOAD %.0f%%", load * 100.0))
            Text(String(format: "IMPACT %.0f%%", impact * 100.0))
            Text(String(format: "SYMM %.0f%%", symmetryProgress * 100.0))
        }
        .font(.system(size: 9, weight: .black, design: .monospaced))
        .foregroundStyle(.white)
        .padding(8)
        .background(Color.black.opacity(0.78))
        .overlay(RoundedRectangle(cornerRadius: 8).stroke(statusColor.opacity(0.55), lineWidth: 1))
        .clipShape(RoundedRectangle(cornerRadius: 8))
    }
}

private enum AVOTrainingSurfaceMode: CaseIterable {
    case load, impact, symmetry, view3D
    ...
    var color: Color {
        ...
        case .view3D: return .purple
        }
    }
}

private struct AVOTrainingLoadSurface: View {
    let values: [Double]
    let impacts: [Double]
    let symmetryProgress: Double
    ...
    var body: some View {
        Canvas { context, size in
            ...
            let rows = 13
            let cols = max(values.count, 14)
            let baseY = height * 0.80
            let xStep = (width * min(max(zoom, 0.72), 1.85)) / CGFloat(cols + 2)
            let yStep = height * 0.040
            let depthX = width * 0.018
            let leftPad = width * 0.020
            ...
            for row in 0..<rows {
                var line = Path()
                for col in 0..<cols {
                    let sourceIndex = min(col, max(values.count - 1, 0))
                    let v = values.isEmpty ? 0.22 : values[sourceIndex]
                    let impact = impacts.isEmpty ? 0.0 : impacts[min(sourceIndex, impacts.count - 1)]
                    let wave = sin(Double(col) * 0.62 + Double(row) * 0.85) * 0.13
                    let symmetryTilt = (0.92 - max(0.0, min(1.0, symmetryProgress))) * Double(row - rows / 2) * 0.018
                    let peak = max(0.04, min(1.0, v + impact * 0.16 + wave + symmetryTilt))
                    let x = leftPad + CGFloat(col + 1) * xStep + CGFloat(row) * depthX - max(0, (CGFloat(cols) * xStep - width) * 0.45)
                    let y = baseY - CGFloat(peak) * height * 0.48 - CGFloat(row) * yStep
                    let point = CGPoint(x: x, y: y)
                    if row == 0 { topPoints.append(point) }
                    if col == 0 { line.move(to: point) } else { line.addLine(to: point) }
                }
                let lineColor: Color = row % 4 == 0 ? .green.opacity(0.74) : .cyan.opacity(0.42)
                context.stroke(line, with: .color(lineColor), lineWidth: row % 4 == 0 ? 1.25 : 0.78)
            }
            ...
            for col in stride(from: 0, to: cols, by: 3) {
                ...
                for row in 0..<rows {
                    ...
                    let impact = impacts.isEmpty ? 0.0 : impacts[min(sourceIndex, impacts.count - 1)]
                    let wave = sin(Double(col) * 0.62 + Double(row) * 0.85) * 0.13
                    let peak = max(0.04, min(1.0, v + impact * 0.16 + wave))
                    let x = leftPad + CGFloat(col + 1) * xStep + CGFloat(row) * depthX - max(0, (CGFloat(cols) * xStep - width) * 0.45)
                    let y = baseY - CGFloat(peak) * height * 0.48 - CGFloat(row) * yStep
                    if row == 0 { line.move(to: CGPoint(x: x, y: y)) } else { line.addLine(to: CGPoint(x: x, y: y)) }
                }
                context.stroke(line, with: .color(.cyan.opacity(0.24)), lineWidth: 0.7)
            }

            if topPoints.count > 2 {
                fillPath.move(to: CGPoint(x: topPoints[0].x, y: baseY + 10))
                for point in topPoints { fillPath.addLine(to: point) }
                fillPath.addLine(to: CGPoint(x: topPoints.last?.x ?? width, y: baseY + 10))
                fillPath.closeSubpath()
                context.fill(fillPath, with: .linearGradient(Gradient(colors: [.cyan.opacity(0.16), .green.opacity(0.08), .clear]), startPoint: CGPoint(x: 0, y: 0), endPoint: CGPoint(x: 0, y: height)))
            }

            let symmetryY = baseY - CGFloat(max(0.05, min(1.0, symmetryProgress))) * height * 0.45
            var symmetryLine = Path()
            symmetryLine.move(to: CGPoint(x: width * 0.06, y: symmetryY))
            symmetryLine.addLine(to: CGPoint(x: width * 0.94, y: symmetryY - (0.92 - CGFloat(max(0.0, min(1.0, symmetryProgress)))) * height * 0.12))
            context.stroke(symmetryLine, with: .color(.green.opacity(0.92)), style: StrokeStyle(lineWidth: 1.5, lineCap: .round, dash: [5, 5]))

            for (idx, impact) in impacts.enumerated() where impact > 0.64 {
                let col = min(idx, cols - 1)
                let row = rows / 2
                let x = leftPad + CGFloat(col + 1) * xStep + CGFloat(row) * depthX - max(0, (CGFloat(cols) * xStep - width) * 0.45)
                let y = baseY - CGFloat(values[min(col, max(values.count - 1, 0))]) * height * 0.48 - CGFloat(row) * yStep
                let radius = CGFloat(12 + impact * 18)
                context.fill(Path(ellipseIn: CGRect(x: x - radius, y: y - radius * 0.62, width: radius * 2, height: radius * 1.24)), with: .color((impact > 0.85 ? Color.red : Color.orange).opacity(0.26)))
                context.fill(Path(ellipseIn: CGRect(x: x - 5, y: y - 4, width: 10, height: 8)), with: .color((impact > 0.85 ? Color.red : Color.orange).opacity(0.92)))
            }

            if let selectedIndex {
                let col = min(max(selectedIndex, 0), cols - 1)
                let x = leftPad + CGFloat(col + 1) * xStep + CGFloat(rows / 2) * depthX - max(0, (CGFloat(cols) * xStep - width) * 0.45)
                var cursor = Path()
                cursor.move(to: CGPoint(x: x, y: height * 0.10))
                cursor.addLine(to: CGPoint(x: x, y: height * 0.90))
                context.stroke(cursor, with: .color(.white.opacity(0.76)), style: StrokeStyle(lineWidth: 1, dash: [3, 3]))
            }

            let border = Path(roundedRect: CGRect(x: 1, y: 1, width: width - 2, height: height - 2), cornerRadius: 8)
            context.stroke(border, with: .color(warningColor.opacity(0.20)), lineWidth: 1)
        }
        .background(
            ZStack {
                LinearGradient(colors: [Color.cyan.opacity(0.10), Color.blue.opacity(0.07), Color.clear], startPoint: .bottomLeading, endPoint: .topTrailing)
                RadialGradient(colors: [warningColor.opacity(0.18), .clear], center: .center, startRadius: 10, endRadius: 210)
                RoundedRectangle(cornerRadius: 8).stroke(Color.cyan.opacity(0.08), lineWidth: 1)
            }
        )
        .clipShape(RoundedRectangle(cornerRadius: 8))
    }
...
private struct AVOTrainingLoadSideStats: View {
    let symmetryText: String
    let symmetrySubtitle: String
    let symmetryColor: Color
    let impactText: String
    let impactSubtitle: String
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(impactSubtitle)
                .font(.system(size: 9, weight: .black, design: .monospaced))
                .foregroundStyle(impactColor.opacity(0.78))
                .lineLimit(1)
        }
        .padding(12)
        .background(
            LinearGradient(colors: [Color.black.opacity(0.48), symmetryColor.opacity(0.06), Color.black.opacity(0.38)], startPoint: .topLeading, endPoint: .bottomTrailing)
        )
        .overlay(RoundedRectangle(cornerRadius: 9).stroke(symmetryColor.opacity(0.24), lineWidth: 1))
        .clipShape(RoundedRectangle(cornerRadius: 9))
    }
}
