...
struct AVOLiveTrainingDashboardPage: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // v1.4.5 BUILD60: do NOT auto-sync sessions on dashboard open.
    ...
    private var liveBottomNavigation: some View {
        HStack(spacing: 0) {
            ...
            Button {
                selectedTab = "SESSION"
                selectedSessionHorseId = canonicalSessionHorseId("HORSE_001")
                // v1.4.5 BUILD60: opening SESSION must stay local-only.
                loadLocalTrainingSessionsForSelectedHorse(status: "LOCAL CACHE · TAP REFRESH TO SYNC")
            } label: {
    ...
    // v1.4.5 BUILD60: storage and Raspberry sync are keyed by the stable server horseId.
    ...
    private func appLocalRootURL() -> URL {
        if let root = stableStore.rootFolderURL {
            // v1.4.5 BUILD60: always use the configured iPad folder selected in SETTINGS > APP FOLDER.
    ...
    private func downloadRemoteSessionDetailData(sessionId: String) async -> Data? {
        ...
        // v1.4.5 BUILD60: Raspberry exposes the detail endpoint as /api/session/<sessionId>.
