            ...
            VStack(alignment: .leading, spacing: 10) {
                if let session = selectedRemoteSession {
                    ...
                    Button {
                        ...
                        analysisSessionToOpen = session
                    } label: {
                        HStack {
                            Image(systemName: "waveform.path.ecg.rectangle")
                            Text("ANÁLISIS COMPLETO DE SESIÓN")
                            Spacer()
                            Text("MAPA · GRÁFICAS · TIMELINE · PDF")
                        }
                        .font(.system(size: 12, weight: .black, design: .monospaced))
...
struct AVOTrainingSessionAnalysisPage: View {
    ...
    private var allSamples: [AVOTrainingAnalysisSample] {
        if !session.samples.isEmpty { return session.samples }
        return session.track.enumerated().map { index, coord in
