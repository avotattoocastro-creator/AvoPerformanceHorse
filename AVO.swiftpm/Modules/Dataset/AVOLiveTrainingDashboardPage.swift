...
struct AVOLiveTrainingDashboardPage: View {
    ...
    private var rtkFixLabel: String {
        ...
        if isDashboardDataFrozen { return "FROZEN" }
        ...
        if raw.contains("3D") || raw.contains("GPS") { return raw.isEmpty ? "GPS 3D" : raw }
    ...
    private var rtkSignalColor: Color {
        if isDashboardDataFrozen || !hardware.vestIsConnected { return .red }
        if rtkFixLabel.contains("RTK FIX") { return .green }
    ...
    private var connectionStatusText: String {
        if isDashboardDataFrozen { return "DATA\nFROZEN" }
        if hardware.vestIsConnected && hardware.cloudStatus.uppercased().contains("ONLINE") { return "CLOUD\nCONNECTED" }
    ...
    private var connectionColor: Color {
        if isDashboardDataFrozen { return .red }
    ...
    private var liveZoneHeaderTitle: String {
        if isDashboardDataFrozen { return "DATA FROZEN" }
    ...
    private var sessionStatusText: String {
        if isDashboardDataFrozen || !hardware.vestIsConnected { return "SESSION STOP" }
        if hardware.isInsideTrainingZone { return "GRABANDO" }
        if sessionSavedAfterExit { return "GUARDADA" }
        return "SESSION STOP"
    }
    ...
    private func refreshDashboardSessionState() {
        if hardware.isInsideTrainingZone && !isDashboardDataFrozen && hardware.vestIsConnected {
    ...
    private func openRemoteSessionAnalysis(_ session: AVORemoteTrainingSession) {
        ...
        guard !session.id.isEmpty,
              let safe = session.id.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed),
              let url = URL(string: "https://live.avoperformance.org/api/session/\(safe)") else {
        ...
        Task {
            do {
                ...
                guard (200...299).contains(code), !data.isEmpty else {
                    await MainActor.run {
                        isLoadingSessionAnalysis = false
                        analysisLoadStatus = "ANALYSIS HTTP \(code)"
                    }
                    return
                }
                let json = try JSONSerialization.jsonObject(with: data)
                ...
                let detail = optimizedSessionForAnalysis(parsedDetail ?? session)
                await MainActor.run {
                    if let idx = remoteTrainingSessions.firstIndex(where: { $0.id == detail.id || $0.id == session.id }) {
                    ...
                    analysisSessionToOpen = detail
                    print("AVO SESSION ANALYSIS DETAIL LOADED", detail.id, detail.samples.count, detail.track.count)
                }
            } catch {
                await MainActor.run {
                    isLoadingSessionAnalysis = false
                    analysisLoadStatus = "ANALYSIS ERROR"
                }
                print("AVO SESSION ANALYSIS DETAIL ERROR", error)
            }
        }
    }
                ...
                if remoteSessionsForSelectedHorse.isEmpty {
                    ...
                    ScrollView {
                        LazyVStack(spacing: 8) {
                            ForEach(Array(remoteSessionsForSelectedHorse.prefix(120))) { session in
                                remoteSessionRow(session)
                            }
                            if remoteSessionsForSelectedHorse.count > 120 {
                                Text("Mostrando 120 sesiones recientes de \(remoteSessionsForSelectedHorse.count). Usa el selector de caballo o refresca para acotar.")
                                    .font(.system(size: 10, weight: .black, design: .monospaced))
                                    .foregroundStyle(.white.opacity(0.52))
                                    .padding(.vertical, 8)
                            }
            ...
            VStack(alignment: .leading, spacing: 10) {
                if let session = selectedRemoteSession {
                    ...
                    RaspberryHorseMapView(
                        ...
                        path: avoDownsampleCoordinatesForUI(session.track, limit: 120),
                        zone: settings.trainingZone
                    )
                    .frame(height: 210)
