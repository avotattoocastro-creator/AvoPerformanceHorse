...
struct AVOLiveTrainingDashboardPage: View {
    ...
    @State private var activeBottomSheet: String?
    ...
    @State private var lastTelemetryDate = Date()
    @State private var didNotifyFrozenData = false
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                ...
                VStack(spacing: 10) {
                    connectionBanner
                    adaptiveHeader(width: geo.size.width)
                        .frame(height: 50)
        ...
        .onAppear {
            ...
            updateTelemetryWatchdog(force: true)
        }
        .onReceive(zoneTimer) { _ in
            hardware.updateTrainingZonePresence(settings.trainingZone)
            updateTelemetryWatchdog(force: false)
        }
        ...
        .fullScreenCover(isPresented: Binding(get: { activeBottomSheet != nil }, set: { if !$0 { activeBottomSheet = nil } })) {
            AVOLiveMenuUtilityPage(
                kind: activeBottomSheet ?? "LIVE",
                hardware: hardware,
                stableStore: stableStore,
                settings: settings,
                onClose: { activeBottomSheet = nil },
                openGeofence: {
                    activeBottomSheet = nil
                    showGeofenceEditor = true
                }
            )
        }
    }
    ...
    private var connectionBanner: some View {
        HStack(spacing: 10) {
            ...
            Spacer()
            Text("ACTIVE VEST HORSE: \(activeHorseName)")
                .font(.system(size: 12, weight: .black, design: .monospaced))
        ...
        .overlay(RoundedRectangle(cornerRadius: 9).stroke(bannerColor.opacity(0.36), lineWidth: 1))
        .clipShape(RoundedRectangle(cornerRadius: 9))
    }

    private func adaptiveHeader(width: CGFloat) -> some View {
    ...
    private var fullHeader: some View {
        HStack(spacing: 10) {
            ...
            liveHeaderBox("RIDER", riderName, .white)
            liveHeaderBox("SESSION", liveSessionDurationText, .white)
            ...
            .background(Color.black.opacity(0.62))
            .overlay(RoundedRectangle(cornerRadius: 10).stroke(cloudColor.opacity(0.25), lineWidth: 1))
            .clipShape(RoundedRectangle(cornerRadius: 10))

            liveHeaderBox("BATTERY", cleanBatteryText, batteryColor)
                .frame(width: 96)
            ...
            AVOCommercialCloseButton(action: { dismiss() })
        }
    }
    ...
    private var compactHeader: some View {
        HStack(spacing: 8) {
            ...
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 1) {
                    Text("LIVE TRAINING")
                        ...
                        .foregroundStyle(.green)
                    Text("HORSE: \(activeHorseName) · \(liveSessionDurationText)")
            ...
            liveHeaderBox("CLOUD", cloudConnectionText, cloudColor)
                .frame(width: 108)
            liveHeaderBox("BAT", cleanBatteryText, batteryColor)
                .frame(width: 82)
    ...
    private func liveDashboardExact(geo: GeometryProxy) -> some View {
        let compact = geo.size.width < 980 || geo.size.height < 720
        return VStack(spacing: compact ? 6 : 8) {
            if compact {
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 8) {
                        liveMapCard
                            .frame(height: max(240, min(360, geo.size.height * 0.40)))
                        liveQuickMetricsRow
                            .frame(height: 56)
                        liveHorseStatusRow
                            .frame(height: 78)
                ...
                HStack(spacing: 10) {
                    VStack(spacing: 8) {
                        liveMapCard
                            .frame(height: max(260, min(400, geo.size.height * 0.39)))
                        liveQuickMetricsRow
                            .frame(height: 56)
                        liveHorseStatusRow
                            .frame(height: 78)
                        liveTrainingLoadRow
                            .frame(height: max(142, min(162, geo.size.height * 0.16)))
                    }
                    .frame(width: max(480, min(geo.size.width * 0.56, geo.size.width - 500)))

                    VStack(spacing: 8) {
                        liveMetricStack
                            .frame(height: max(230, min(318, geo.size.height * 0.30)))
                        Spacer(minLength: 0)
                        horseRiderGravityField
                            .frame(height: max(260, min(390, geo.size.height * 0.39)))
                    }
    ...
    private var liveMapCard: some View {
        ZStack(alignment: .topLeading) {
            RaspberryHorseMapView(
                ...
                isInsideTrainingZone: hardware.isInsideTrainingZone,
                signalColor: mapNeonColor
            )
        ...
        .overlay(RoundedRectangle(cornerRadius: 10).stroke(mapNeonColor.opacity(0.95), lineWidth: isFrozen ? 3 : 2))
        .shadow(color: mapNeonColor.opacity(0.42), radius: 10)
        .clipShape(RoundedRectangle(cornerRadius: 10))
    }
    ...
    private var liveQuickMetricsRow: some View {
        HStack(spacing: 6) {
            ...
            metricCard("CADENCE", cleanCadenceText, zoneMetricColor)
    ...
    private var liveHorseStatusRow: some View {
        HStack(spacing: 0) {
            ...
            liveStatusBlock(icon: "location.fill", title: "GPS STATUS", value: gpsStatusText + "\n" + rtkAccuracyText, color: gpsColor)
            liveStatusBlock(icon: hardware.isInsideTrainingZone ? "checkmark" : "exclamationmark.triangle.fill", title: "ALERTS", value: alertStatusText, color: zoneMetricColor)
        }
    ...
    private var liveMetricStack: some View {
        VStack(spacing: 8) {
            liveLargeMetric(icon: "heart", title: "HEART RATE", value: heartValue, unit: "BPM", color: .green, values: hardware.heartHistory)
            liveLargeMetric(icon: "speedometer", title: "SPEED", value: speedValue, unit: "km/h", color: .cyan, values: hardware.speedHistory)
            liveLargeMetric(icon: "figure.equestrian.sports", title: "GAIT", value: gaitValue, unit: "RIGHT LEAD", color: .orange, values: hardware.impactHistory)
        }
    }
    ...
    private var liveBottomNavigation: some View {
        HStack(spacing: 0) {
            liveNavButton("waveform.path.ecg", "LIVE", selected: true) { selectedTab = "LIVE" }
            liveNavButton("clock", "SESSION", selected: activeBottomSheet == "SESSION") { activeBottomSheet = "SESSION" }
            liveNavButton("gauge", "HISTORY", selected: activeBottomSheet == "HISTORY") { activeBottomSheet = "HISTORY" }
            liveNavButton("figure.equestrian.sports", "HORSES", selected: activeBottomSheet == "HORSES") { activeBottomSheet = "HORSES" }
            liveNavButton("person", "RIDERS", selected: activeBottomSheet == "RIDERS") { activeBottomSheet = "RIDERS" }
            liveNavButton("bell", "ALERTS", selected: activeBottomSheet == "ALERTS") { activeBottomSheet = "ALERTS" }
            liveNavButton("mappin.and.ellipse", "GEOFENCE", selected: false, color: .green) { showGeofenceEditor = true }
            liveNavButton("gearshape", "SETTINGS", selected: false) { showDashboardSettings = true }
        }
    ...
    private var liveVersionBrandFooter: some View {
        ViewThatFits(in: .horizontal) {
            liveVersionBrandFooterFull
            liveVersionBrandFooterCompact
            ScrollView(.horizontal, showsIndicators: false) {
                liveVersionBrandFooterFull
                    .frame(minWidth: 1380)
            }
    ...
    private var liveVersionBrandFooterFull: some View {
        HStack(spacing: 8) {
            versionFooterBox(width: 140, fontSize: 32)
            brandFooterBox
            footerInfoBox("ACTUALIZACIÓN", "27/05/2026\nBUILD 25", .white)
            ...
            newsFooterBox(width: 310)
        }
    }

    private var liveVersionBrandFooterCompact: some View {
        HStack(spacing: 6) {
            versionFooterBox(width: 112, fontSize: 28)
            brandFooterBox
            footerInfoBox("BUILD", "25", .white)
                .frame(width: 82)
            footerInfoBox("BAT", cleanBatteryText, batteryColor)
                .frame(width: 96)
            footerInfoBox("LINK", isFrozen ? "FROZEN" : (hardware.vestIsConnected ? "ONLINE" : "WAIT"), hardware.vestIsConnected && !isFrozen ? .green : .orange)
                .frame(width: 100)
        }
    }

    private func versionFooterBox(width: CGFloat, fontSize: CGFloat) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("VERSIÓN")
                .font(.system(size: 10, weight: .black, design: .monospaced))
                .foregroundStyle(.green)
    ...
    private var brandFooterBox: some View {
        VStack(alignment: .leading, spacing: 6) {
    ...
    private func newsFooterBox(width: CGFloat) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("NOVEDADES v1.1.2")
                .font(.system(size: 12, weight: .black, design: .monospaced))
                .foregroundStyle(.cyan)
            Text("""
• Anti-spam real para datos congelados
• Layout responsive iPad / landscape
• Borde neón GPS por estado
• Cámara fija dentro de geocerca
• Limpieza Codemagic/CoreML build 25
""")
    ...
    private var currentTelemetrySignature: String {
        String(format: "%.6f|%.6f|%@|%@|%.2f|%.2f", hardware.externalCoordinate.latitude, hardware.externalCoordinate.longitude, hardware.speed, hardware.pulse, hardware.imuPitch, hardware.imuRoll)
    }

    private var isFrozen: Bool {
        Date().timeIntervalSince(lastTelemetryDate) > 6
    }

    private var bannerText: String {
        if isFrozen { return "Chaleco desconectado · datos congelados" }
        let raw = hardware.vestConnectionAlert.trimmingCharacters(in: .whitespacesAndNewlines)
        return raw.isEmpty ? (hardware.vestIsConnected ? "Chaleco conectado por CLOUD" : "Chaleco desconectado") : raw
    }

    private var bannerColor: Color {
        if isFrozen { return .red }
        return hardware.vestIsConnected ? .green : .orange
    }

    private var riderName: String {
        let raw = hardware.activeVestRider.trimmingCharacters(in: .whitespacesAndNewlines)
        return raw.isEmpty || raw == "--" ? "NO RIDER" : raw
    }

    private var zoneMetricColor: Color {
        if isFrozen { return .red }
        return hardware.isInsideTrainingZone ? .green : .orange
    }

    private var mapNeonColor: Color {
        if isFrozen || !hardware.vestIsConnected { return .red }
        let weakGps = hardware.gpsSatellites > 0 && hardware.gpsSatellites < 8
        let weakCloud = hardware.cloudEnabled && !hardware.cloudStatus.uppercased().contains("ONLINE")
        let weakFix = hardware.gpsFix == "NO_FIX" || hardware.gpsHDOP > 3.0
        return (weakGps || weakCloud || weakFix) ? .orange : .green
    }

    private var cloudConnectionText: String {
        if isFrozen { return "FROZEN" }
        let online = hardware.cloudStatus.uppercased().contains("ONLINE") || hardware.cloudEnabled
        return online && hardware.vestIsConnected ? "CONNECTED" : online ? "ONLINE" : "OFFLINE"
    }

    private var cloudColor: Color {
        if isFrozen { return .red }
        return cloudConnectionText == "OFFLINE" ? .orange : .green
    }

    private var connectionColor: Color {
        if isFrozen { return .red }
        return connectionStatusText.contains("CONNECTED") ? .green : .orange
    }

    private var gpsColor: Color {
        if isFrozen { return .red }
        return gpsStatusText.contains("FIX") || gpsStatusText.contains("GPS") ? .green : .orange
    }

    private var connectionStatusText: String {
        if isFrozen { return "DATA\nFROZEN" }
    ...
    private var alertStatusText: String {
        if isFrozen { return "DATA\nFROZEN" }
        return hardware.isInsideTrainingZone ? "✓ NONE" : "OUTSIDE\nZONE"
    }
    ...
    private func liveNavButton(_ icon: String, _ title: String, selected: Bool, color: Color = .white, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ...
            .font(.system(size: 11, weight: .black, design: .monospaced))
            .foregroundStyle(selected ? .green : (color == .green ? .green : .white.opacity(0.62)))
            .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    private func updateTelemetryWatchdog(force: Bool) {
        ...
        if force || signature != lastTelemetrySignature {
            ...
            didNotifyFrozenData = false
            return
        }

        if Date().timeIntervalSince(lastTelemetryDate) > 6, !didNotifyFrozenData {
            // UI-only freeze state. The hardware receiver owns the real push notification
            // so the dashboard never creates duplicated frozen-data banners.
            didNotifyFrozenData = true
        }
    }
...
struct RaspberryHorseMapView: View {
    ...
    let isInsideTrainingZone: Bool
    let signalColor: Color

    var body: some View {
        // Inside the geofence: fixed camera centered on saved geofence. Outside: dynamic follow.
        let cameraCenter = isInsideTrainingZone ? zone.coordinate : coordinate
        let fixedLatDelta = max(0.00055, min(0.012, zone.fixedCameraLatitudeDelta))
        let fixedLonDelta = max(0.00055, min(0.012, zone.fixedCameraLongitudeDelta))
        let cameraSpan = isInsideTrainingZone ? MKCoordinateSpan(latitudeDelta: fixedLatDelta, longitudeDelta: fixedLonDelta) : MKCoordinateSpan(latitudeDelta: 0.010, longitudeDelta: 0.010)
        ...
        .clipShape(RoundedRectangle(cornerRadius: 8))
        .overlay(RoundedRectangle(cornerRadius: 8).stroke(signalColor.opacity(0.95), lineWidth: 2.4))
        .shadow(color: signalColor.opacity(0.35), radius: 13, x: 0, y: 0)
    }
}
...
final class AVOTrainingPushBridge: NSObject, UNUserNotificationCenterDelegate {
    ...
    private var lastDeliveredByCategory: [String: Date] = [:]
    ...
    func deliver(title: String, body: String) {
        ...
        let category: String
        if lowerKey.contains("congel") || lowerKey.contains("frozen") || lowerKey.contains("stale") {
            category = "frozen"
        } else if lowerKey.contains("desconect") || lowerKey.contains("offline") || lowerKey.contains("sin datos") || lowerKey.contains("no data") {
            category = "disconnect"
        } else if lowerKey.contains("caballo cargado") || lowerKey.contains("horse loaded") {
            category = "horse_loaded"
        } else {
            category = key
        }
        ...
        switch category {
        case "frozen", "disconnect": minimumInterval = 1800.0
        case "horse_loaded": minimumInterval = 3600.0
        default: minimumInterval = 10.0
        }

        if let last = lastDeliveredByCategory[category], now.timeIntervalSince(last) < minimumInterval { return }
        if key == lastDeliveredKey && now.timeIntervalSince(lastDeliveredDate) < minimumInterval { return }
        lastDeliveredByCategory[category] = now
        lastDeliveredKey = key
        lastDeliveredDate = now

        center.removePendingNotificationRequests(withIdentifiers: ["avo-training-\(category)"])
        ...
        let request = UNNotificationRequest(
            identifier: "avo-training-\(category)",
            content: content,
...
// MARK: - LIVE bottom menu utility pages

private struct AVOLiveMenuUtilityPage: View {
    let kind: String
    @ObservedObject var hardware: AVOHardwareReceiver
    @ObservedObject var stableStore: AVOStableStore
    @ObservedObject var settings: HardwareSettings
    let onClose: () -> Void
    let openGeofence: () -> Void

    var body: some View {
        ZStack {
            LinearGradient(colors: [Color.black, Color(red: 0.012, green: 0.020, blue: 0.018)], startPoint: .topLeading, endPoint: .bottomTrailing)
                .ignoresSafeArea()
            VStack(spacing: 12) {
                header
                content
    ...
    private var header: some View {
        HStack(spacing: 12) {
            Text(kind)
                .font(.system(size: 28, weight: .black, design: .monospaced))
                .foregroundStyle(.green)
            Text("LIVE TRAINING MODULE")
                .font(.system(size: 11, weight: .black, design: .monospaced))
                .foregroundStyle(.cyan)
            Spacer()
            Text(hardware.vestIsConnected ? "VEST CONNECTED" : "VEST DISCONNECTED")
                .font(.system(size: 12, weight: .black, design: .monospaced))
                .foregroundStyle(hardware.vestIsConnected ? .green : .red)
                .padding(.horizontal, 12)
                .padding(.vertical, 9)
                .overlay(RoundedRectangle(cornerRadius: 10).stroke((hardware.vestIsConnected ? Color.green : Color.red).opacity(0.4), lineWidth: 1))
    ...
    @ViewBuilder private var content: some View {
        ...
        case "HORSES": horsesContent
        case "RIDERS": ridersContent
        case "ALERTS": alertsContent
        default: sessionContent
        }
    }
    ...
    private var sessionContent: some View {
        ScrollView {
            VStack(spacing: 10) {
                livePanel("ACTIVE SESSION") {
                    metric("HORSE", stableStore.selectedHorseName, .green)
                    metric("RIDER", hardware.activeVestRider, .cyan)
                    metric("CONNECTION", hardware.vestConnectionState, hardware.vestIsConnected ? .green : .red)
                    metric("ZONE", hardware.trainingZonePresence, hardware.isInsideTrainingZone ? .green : .orange)
                    metric("AUTO RECORD", settings.autoRecordInsideZone ? "ON INSIDE GEOFENCE" : "MANUAL", settings.autoRecordInsideZone ? .green : .orange)
                }
                livePanel("REALTIME SESSION DATA") {
                    metric("HEART", hardware.pulse, .green)
                    metric("SPEED", hardware.speed, .cyan)
                    metric("GAIT", hardware.gaitState, .orange)
                    metric("IMPACT", String(format: "%.2f G", hardware.imuImpact), .orange)
                    metric("PACKETS", hardware.packetStatus, .green)
                }
            }
        }
    }
    ...
    private var historyContent: some View {
        ScrollView {
            VStack(spacing: 10) {
                livePanel("RECENT STABLE SESSIONS") {
                    if stableStore.selectedSessions.isEmpty {
                        metric("STATUS", "NO SAVED SESSIONS FOR THIS HORSE", .orange)
                    } else {
                        ForEach(stableStore.selectedSessions.prefix(12)) { item in
                            metric(item.title, String(format: "%.0fs · Q %.0f%% · FAT %.0f%%", item.durationSeconds, item.avgQuality * 100, item.avgFatigue * 100), .cyan)
                        }
                    }
                }
            }
    ...
    private var horsesContent: some View {
        ScrollView {
            VStack(spacing: 10) {
                livePanel("HORSES") {
                    if stableStore.horsesIndex.isEmpty {
                        metric("STABLE", "NO HORSES CREATED", .orange)
                    } else {
                        ForEach(stableStore.horsesIndex) { horse in
                            metric(horse.name, "\(horse.breed) · \(horse.ageYears)y · \(horse.competitionMode)", horse.name == stableStore.selectedHorseName ? .green : .white)
                        }
                    }
                }
                Button(action: openGeofence) {
                    Text("OPEN GEOFENCE FOR SELECTED HORSE")
                        .font(.system(size: 13, weight: .black, design: .monospaced))
                        .foregroundStyle(.black)
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 14)
    ...
    private var ridersContent: some View {
        ScrollView {
            livePanel("RIDER / VEST") {
                metric("ACTIVE RIDER", hardware.activeVestRider, .cyan)
                metric("NFC RIDER", hardware.nfcRider, .cyan)
                metric("VEST BATTERY", hardware.remoteBattery, .green)
                metric("IMU PITCH", String(format: "%.1f°", hardware.imuPitch), .cyan)
                metric("IMU ROLL", String(format: "%.1f°", hardware.imuRoll), .cyan)
                metric("RIDER SENSOR", hardware.vestIsConnected ? "LIVE FROM VEST" : "DISCONNECTED", hardware.vestIsConnected ? .green : .red)
            }
        }
    }
    ...
    private var alertsContent: some View {
        ScrollView {
            VStack(spacing: 10) {
                livePanel("LIVE ALERTS") {
                    metric("LAST", hardware.latestNotificationTitle, .orange)
                    metric("DETAIL", hardware.latestNotificationBody, .white)
                    metric("ZONE", hardware.trainingZonePresence, hardware.isInsideTrainingZone ? .green : .orange)
                    metric("GPS", hardware.gpsFix, hardware.gpsFix.contains("RTK") || hardware.gpsFix.contains("GPS") ? .green : .orange)
                    metric("CLOUD", hardware.cloudStatus, hardware.cloudStatus.contains("ONLINE") ? .green : .red)
                }
                livePanel("EVENT FEED") {
                    if hardware.notificationFeed.isEmpty {
                        metric("EVENTS", "NO EVENTS YET", .orange)
                    } else {
                        ForEach(hardware.notificationFeed.prefix(20)) { event in
                            metric(event.title, event.body, event.type.contains("outside") || event.type.contains("disconnected") ? .orange : .green)
                        }
                    }
                }
            }
    ...
    private func livePanel<Content: View>(_ title: String, @ViewBuilder content: () -> Content) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(title)
                .font(.system(size: 16, weight: .black, design: .monospaced))
                .foregroundStyle(.green)
    ...
    private func metric(_ name: String, _ value: String, _ color: Color) -> some View {
