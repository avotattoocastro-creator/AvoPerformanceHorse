...
struct AVOLiveTrainingDashboardPage: View {
    ...
    private func liveDashboardExact(geo: GeometryProxy) -> some View {
        VStack(spacing: 8) {
            HStack(spacing: 10) {
                VStack(spacing: 8) {
                    liveMapCard
                        .frame(height: max(280, min(420, geo.size.height * 0.42)))
                    liveQuickMetricsRow
                        .frame(height: 56)
                    liveHorseStatusRow
                        .frame(height: 78)
                ...
                .frame(width: max(540, min(geo.size.width * 0.57, geo.size.width - 520)))

                VStack(spacing: 8) {
                    liveMetricStack
                        .frame(height: max(245, min(340, geo.size.height * 0.32)))
                    Spacer(minLength: 0)
    ...
    private var liveBottomNavigation: some View {
        HStack(spacing: 0) {
            ...
            liveNavButton("clock", "SESSION", selected: false) { activeBottomSheet = "SESSION" }
            liveNavButton("gauge", "HISTORY", selected: false) { activeBottomSheet = "HISTORY" }
            liveNavButton("figure.equestrian.sports", "HORSES", selected: false) { activeBottomSheet = "HORSES" }
            liveNavButton("person", "RIDERS", selected: false) { activeBottomSheet = "RIDERS" }
            liveNavButton("bell", "ALERTS", selected: false) { activeBottomSheet = "ALERTS" }
...
struct RaspberryHorseMapView: View {
    ...
    var body: some View {
        ...
        let cameraSpan = isInsideTrainingZone ? MKCoordinateSpan(latitudeDelta: 0.006, longitudeDelta: 0.006) : MKCoordinateSpan(latitudeDelta: 0.010, longitudeDelta: 0.010)
...
final class AVOTrainingPushBridge: NSObject, UNUserNotificationCenterDelegate {
    ...
    func deliver(title: String, body: String) {
        ...
        let isFrozenOrDisconnected = lowerKey.contains("congel") || lowerKey.contains("desconect") || lowerKey.contains("frozen")
        let minimumInterval: TimeInterval = isFrozenOrDisconnected ? 1800.0 : 8.0
        if key == lastDeliveredKey && now.timeIntervalSince(lastDeliveredDate) < minimumInterval {
            return
        }
