...
final class AVODashboardSettingsStore: ObservableObject {
    ...
    init() {
        ...
        // v1.3.5 BUILD50: fuerza una migración limpia a CLOUD REAL.
        ...
        let migrationKey = "AVO.dashboard.cloudOnlyMigration.1.3.5"
        if defaults.bool(forKey: migrationKey) == false {
            ...
            defaults.set("live.avoperformance.org", forKey: "AVO.dashboard.host")
            defaults.set("443", forKey: "AVO.dashboard.port")
            defaults.set("/api/latest/compact", forKey: "AVO.dashboard.path")
            defaults.set(1.0, forKey: "AVO.dashboard.pollRate")
            defaults.set(false, forKey: "AVO.dashboard.udpEnabled")
            defaults.set(true, forKey: migrationKey)
        }

        self.cloudEnabled = defaults.object(forKey: "AVO.dashboard.cloudEnabled") as? Bool ?? true
        self.useHTTPS = defaults.object(forKey: "AVO.dashboard.useHTTPS") as? Bool ?? true
        self.host = defaults.string(forKey: "AVO.dashboard.host") ?? "live.avoperformance.org"
        self.port = defaults.string(forKey: "AVO.dashboard.port") ?? "443"
        self.path = defaults.string(forKey: "AVO.dashboard.path") ?? "/api/latest/compact"
        self.pollRate = defaults.object(forKey: "AVO.dashboard.pollRate") as? Double ?? 1.0
        self.udpEnabled = defaults.object(forKey: "AVO.dashboard.udpEnabled") as? Bool ?? false
    ...
    var resolvedURL: String {
        ...
        if (scheme == "https" && (Int(port) ?? 443) == 443) || (scheme == "http" && (Int(port) ?? 80) == 80) {
...
struct AVODashboardSettingsPage: View {
    ...
    private var connectionPanel: some View {
        settingsBox("RASPBERRY CLOUD SERVER") {
            ToggleRow(title: "Activar Cloud Dashboard", subtitle: "Lee /api/latest/compact desde la Raspberry pública o local.", isOn: $settings.cloudEnabled)
            ...
            FieldRow(title: "Endpoint", text: $settings.path, placeholder: "/api/latest/compact")
