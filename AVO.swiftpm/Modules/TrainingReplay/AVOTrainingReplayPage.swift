...
final class AVOTrainingReplayDetailViewModel: ObservableObject {
    ...
    func load() {
        ...
        Task {
            do {
                // v1.4.5 BUILD60: universal WORK/WATCH replay loader.
                ...
                await MainActor.run {
                    ...
                    self.viewStart = self.minTime
                    self.viewEnd = max(self.minTime + 1.0, self.maxTime)
                    self.isLoading = false
    ...
    func resetViewport() {
        viewStart = minTime
        viewEnd = max(minTime + 1.0, maxTime)
    }
    ...
    func zoomViewport(scale: CGFloat, centerTime: Double) {
        ...
        if newStart < minTime {
            newStart = minTime
            newEnd = min(maxTime, newStart + proposed)
        }
        if newEnd > maxTime {
            newEnd = maxTime
            newStart = max(minTime, newEnd - proposed)
        }
    ...
    func panViewport(deltaSeconds: Double) {
        ...
        if newEnd > maxTime {
            newEnd = maxTime
            newStart = max(minTime, newEnd - span)
        }
...
enum AVOTrainingReplayParser {
    static func parseGPSCSV(data: Data?) -> [AVOReplayGPSPoint] {
        ...
        for row in rows.dropFirst() {
            ...
            out.append(AVOReplayGPSPoint(
                t: double(row, "t", "time", "timestamp", "timestampMs", "ms"),
                lat: lat,
                lon: lon,
    ...
    static func parseGPSJSONL(data: Data?) -> [AVOReplayGPSPoint] {
        ...
        for obj in jsonObjects(text) {
            ...
            if lat == 0 && lon == 0 { continue }
            out.append(AVOReplayGPSPoint(
                t: numeric(obj["t"] ?? obj["time"] ?? obj["timestampMs"] ?? gps["t"] ?? gps["time"]),
                lat: lat,
                lon: lon,
    ...
    static func parseIMUJSONL(data: Data?) -> [AVOReplayIMUSample] {
        ...
        for obj in jsonObjects(text) {
            ...
            if ax == 0 && ay == 0 && az == 0 && gx == 0 && gy == 0 && gz == 0 && mx == 0 && my == 0 && mz == 0 { continue }
            out.append(AVOReplayIMUSample(
                t: numeric(obj["t"] ?? obj["time"] ?? obj["timestampMs"] ?? imu["t"] ?? imu["time"]),
                ax: ax,
                ay: ay,
                az: az,
                gx: gx,
    ...
    private static func normalizeTime(_ gps: [AVOReplayGPSPoint]) -> [AVOReplayGPSPoint] {
        ...
        let firstT = first.t
        let shouldConvertMs = firstT > 10_000
        return gps.map { p in
            let t = shouldConvertMs ? (p.t - firstT) / 1000.0 : p.t
            return AVOReplayGPSPoint(t: t, lat: p.lat, lon: p.lon, altM: p.altM, speedKmh: p.speedKmh, distanceM: p.distanceM, fix: p.fix, sat: p.sat, hAccM: p.hAccM, vAccM: p.vAccM, rtcm: p.rtcm)
        }
    }
    ...
    private static func normalizeTime(_ imu: [AVOReplayIMUSample]) -> [AVOReplayIMUSample] {
        ...
        return imu.map { p in
            let t = shouldConvertMs ? (p.t - firstT) / 1000.0 : p.t
            return AVOReplayIMUSample(t: t, ax: p.ax, ay: p.ay, az: p.az, gx: p.gx, gy: p.gy, gz: p.gz, mx: p.mx, my: p.my, mz: p.mz)
        }
    }
}
