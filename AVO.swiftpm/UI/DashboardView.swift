...
struct DashboardView: View {
    ...
    @State private var showTrainingReplayPage = false
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                ...
                if showHomeLauncher {
                    AVOHomeLauncherView(hardware: hardware) { mode in
                        if mode == .live {
                            ...
                            showReplayPage = true
                        } else if mode == .trainingReplay {
                            showTrainingReplayPage = true
        ...
        .fullScreenCover(isPresented: $showTrainingReplayPage) {
            AVOTrainingReplayListPage(hardware: hardware)
        }
        ...
        .fullScreenCover(isPresented: $showConfigHubPage) {
            AVOServerControlPage(hardware: hardware, settings: settings)
        }
